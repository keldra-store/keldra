...
All workspace packages currently share version `0.8.2`:
...
Anvil 0.8.2 runs as one flat cluster of capacity-weighted nodes with native
...
validated commit is pushed, maintainers publish `0.8.2` with:

```sh
validated_commit="$(git rev-parse HEAD)"
git tag 0.8.2 "$validated_commit"
git push origin refs/tags/0.8.2
```
...
cargo info anvil-api@0.8.2

cargo publish --locked -p anvil-storage
cargo info anvil-storage@0.8.2
```

Do not publish `anvil-storage` until `cargo info anvil-api@0.8.2` resolves from
crates.io. After both commands succeed, run both `cargo info` checks again and
confirm that each reports version `0.8.2` from crates.io.
