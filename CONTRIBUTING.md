...
All workspace packages currently share version `0.9.0`:
...
validated commit is pushed, maintainers publish `0.9.0` with:

```sh
validated_commit="$(git rev-parse HEAD)"
git tag 0.9.0 "$validated_commit"
...
cargo info anvil-api@0.9.0

cargo publish --locked -p anvil-storage
cargo info anvil-storage@0.9.0
```

Do not publish `anvil-storage` until `cargo info anvil-api@0.9.0` resolves from
crates.io. After both commands succeed, run both `cargo info` checks again and
confirm that each reports version `0.9.0` from crates.io.
