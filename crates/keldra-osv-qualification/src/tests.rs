...
#[test]
fn qualification_requires_an_exact_keldra_commit() {
    assert!(validate_git_commit("--keldra-commit", &"a".repeat(40)).is_ok());
    assert!(validate_git_commit("--keldra-commit", &"A".repeat(40)).is_ok());
    let short = "a".repeat(39);
    let non_hex = "g".repeat(40);
    for invalid in ["", "main", short.as_str(), non_hex.as_str()] {
        assert!(validate_git_commit("--keldra-commit", invalid).is_err());
    }
}
