...
const OCI_BINDING_PATH: &str = "_anvil/plugins/oci@1";
...
async fn binding(
    ...
    while let Some(chunk) = stream.next().await {
        match chunk.map_err(PluginError::from_status)?.value {
            Some(anvil_api::v1::object_chunk::Value::Head(_)) if !saw_head => saw_head = true,
            Some(anvil_api::v1::object_chunk::Value::Bytes(next)) if saw_head => {
