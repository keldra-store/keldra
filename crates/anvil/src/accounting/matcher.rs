...
use anvil_store::DefinitionKind;
...
impl MatcherCache {
    fn lookup(&mut self, bucket: BucketIdentity) -> Option<Arc<MatcherRoutes>> {
        ...
        tracing::debug!(
            ...
            gauge.anvil_accounting_matcher_cache_generation = entry.generation,
            gauge.anvil_accounting_matcher_cache_age_millis = age.as_millis() as u64,
            gauge.anvil_accounting_definition_propagation_lag_millis = age.as_millis() as u64,
    ...
    fn insert(
        ...
        tracing::debug!(
            tenant.id = bucket.tenant_id,
            bucket.id = bucket.bucket_id,
            gauge.anvil_accounting_matcher_cache_buckets = self.buckets.len() as u64,
            gauge.anvil_accounting_matcher_cache_definitions = self.definition_count as u64,
            gauge.anvil_accounting_matcher_cache_bytes = self.charged_bytes,
            gauge.anvil_accounting_matcher_cache_generation = self.generation,
