...
use anvil_consensus::{NodeId, PeerSpkiSha256};
use anvil_store::{
...
impl ClusterPeerService {
    ...
    pub(super) async fn fresh_authorization_check_call(
        ...
        if let Some(binding) = raw.stable_bucket.clone() {
            ...
            let tenant = anvil_store::StorageTenantId::parse(&binding.storage_tenant)
...
fn read_candidate(
    repository: &anvil_store::AuthzRepository,
    scope: &AuthzScope,
...
fn installed_response(
    applied: anvil_store::AuthzRealmSnapshotApplied,
) -> Result<Response<wire::RealmCandidateInstalled>, Status> {
