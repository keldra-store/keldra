...
impl PayloadGarbageCollector {
    ...
    pub(crate) async fn run_once(&self) -> Result<PayloadGcTick, PayloadGcError> {
                ...
                if !artifact_is_selected(self.local_node, &artifact.identity, &desired) {
                    ...
                    .await
                    {
                        ...
                        Ok(Err(error)) => {
                            tracing::debug!(
                                %error,
                                monotonic_counter.anvil_payload_gc_probe_skipped_total = 1_u64,
                        ...
                        Err(_) => {
                            tracing::debug!(
                                monotonic_counter.anvil_payload_gc_probe_timeouts_total = 1_u64,
        ...
        tracing::debug!(
            gauge.anvil_payload_gc_tick_records = inspected as u64,
            gauge.anvil_payload_gc_tick_bytes = inspected_bytes,
            monotonic_counter.anvil_payload_gc_tick_retired_total = retired,
