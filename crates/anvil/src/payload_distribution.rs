...
impl PreparedPayloadEvidence {
    ...
    pub(crate) const fn placement_fence(&self) -> anvil_store::PlacementLogId {
...
pub(crate) enum PayloadDistributionError {
    ...
    #[error(transparent)]
    Store(#[from] anvil_store::PayloadStoreError),
    #[error(transparent)]
    Erasure(#[from] anvil_store::ErasureError),
}
...
mod tests {
    ...
    use anvil_store::{ErasureProfile, SMALL_BLOB_MAX_BYTES, StoreOptions};
    ...
    impl PayloadPlacementView for TestPlacement {
        ...
        fn fence(&self) -> anvil_store::PlacementLogId {
            anvil_store::PlacementLogId { term: 4, index: 9 }
        }
