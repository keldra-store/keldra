...
async fn prewrite_repair_replaces_local_minority_before_constructing_successor() {
    ...
    assert_eq!(
        ...
        anvil_store::LogicalRecordPredecessor::VersionId(VersionId(100))
    );
    for replica in replicas {
        assert_eq!(
            fixture.stores[replica]
...
async fn retry_after_lost_response_returns_the_exact_committed_version() {
    ...
    let value = LogicalRecordValue::TenantNameClaim {
        storage_tenant: anvil_store::StorageTenantId::parse("same-command").unwrap(),
        tenant_id: 99,
    };
    let first = fixture
        .core
...
async fn one_process_mutex_serializes_complete_reconcile_mutate_sequences() {
    ...
    assert_eq!(
        ...
        anvil_store::LogicalRecordPredecessor::VersionId(first.record_version)
    );
}
...
fn placement_keys_use_only_the_fixed_rfc_encodings() {
    ...
    assert_eq!(
        placement_key(&LogicalRecordId::TenantSchema {
            storage_tenant: anvil_store::StorageTenantId::parse("tenant").unwrap(),
            schema_ref: anvil_store::SchemaRef {
                schema_id: anvil_store::SchemaId::parse("default").unwrap(),
                schema_revision: 1,
                schema_digest: anvil_store::SchemaDigest([0; 32]),
            },
        })
        .unwrap_err()
        .code(),
