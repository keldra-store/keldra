        ...
        span.in_scope(|| {
            match phase {
                BuilderProgressPhase::Rebuild => tracing::info!(
                    index.kind = ?identity.kind,
                    counter.anvil_index_rebuild_active = 1_i64,
                    monotonic_counter.anvil_index_rebuilds_total = 1_u64,
                    gauge.anvil_index_rebuild_elapsed_seconds = 0_f64,
                    gauge.anvil_index_rebuild_last_progress_age_seconds = 0_f64,
    ...
    fn emit_progress(&self, emission: BuilderProgressEmission, heartbeat: bool) {
        ...
        let emit = || match self.phase {
            BuilderProgressPhase::Rebuild => tracing::info!(
                ...
                monotonic_counter.anvil_index_rebuild_records_total = emission.records,
                monotonic_counter.anvil_index_rebuild_bytes_total = emission.bytes,
                monotonic_counter.anvil_index_rebuild_frames_total = emission.units,
                monotonic_counter.anvil_index_rebuild_progress_heartbeats_total =
                    u64::from(heartbeat),
                gauge.anvil_index_rebuild_elapsed_seconds =
            ...
            BuilderProgressPhase::CatchUp => tracing::info!(
                index.kind = ?self.identity.kind,
                monotonic_counter.anvil_index_catch_up_records_total = emission.records,
                monotonic_counter.anvil_index_catch_up_bytes_total = emission.bytes,
                monotonic_counter.anvil_index_catch_up_pages_total = emission.units,
                monotonic_counter.anvil_index_catch_up_progress_heartbeats_total =
                    u64::from(heartbeat),
                gauge.anvil_index_catch_up_elapsed_seconds =
                    emission.snapshot.elapsed_seconds,
    ...
    fn emit_terminal(&self, emission: BuilderProgressEmission, failed: bool) {
        match self.phase {
            BuilderProgressPhase::Rebuild => tracing::info!(
                ...
                monotonic_counter.anvil_index_rebuild_bytes_total = emission.bytes,
                monotonic_counter.anvil_index_rebuild_frames_total = emission.units,
                monotonic_counter.anvil_index_rebuild_failures_total = u64::from(failed),
                gauge.anvil_index_rebuild_elapsed_seconds = emission.snapshot.elapsed_seconds,
                gauge.anvil_index_rebuild_last_progress_age_seconds =
                    emission.snapshot.last_progress_age_seconds,
                histogram.anvil_index_rebuild_records = emission.snapshot.records,
                histogram.anvil_index_rebuild_bytes = emission.snapshot.bytes,
                histogram.anvil_index_rebuild_frames = emission.snapshot.units,
                histogram.anvil_index_rebuild_duration_seconds =
                    emission.snapshot.elapsed_seconds,
            ...
            BuilderProgressPhase::CatchUp => tracing::info!(
                ...
                monotonic_counter.anvil_index_catch_up_bytes_total = emission.bytes,
                monotonic_counter.anvil_index_catch_up_pages_total = emission.units,
                monotonic_counter.anvil_index_catch_up_failures_total = u64::from(failed),
                gauge.anvil_index_catch_up_elapsed_seconds = emission.snapshot.elapsed_seconds,
                gauge.anvil_index_catch_up_last_progress_age_seconds =
                    emission.snapshot.last_progress_age_seconds,
                histogram.anvil_index_catch_up_records = emission.snapshot.records,
                histogram.anvil_index_catch_up_bytes = emission.snapshot.bytes,
                histogram.anvil_index_catch_up_pages = emission.snapshot.units,
                histogram.anvil_index_catch_up_duration_seconds =
                    emission.snapshot.elapsed_seconds,
...
impl CompactionTelemetry {
    ...
    pub(crate) fn start(
        ...
        span.in_scope(|| {
            tracing::info!(
                index.kind = ?identity.kind,
                counter.anvil_index_compaction_active = 1_i64,
                monotonic_counter.anvil_index_compaction_attempts_total = 1_u64,
                gauge.anvil_index_compaction_configured_lanes =
                    parallelism.configured_lanes() as u64,
                gauge.anvil_index_compaction_worker_limit = parallelism.worker_limit() as u64,
                gauge.anvil_index_compaction_budget_limit = parallelism.budget_limit() as u64,
                gauge.anvil_index_compaction_active_lanes = 0_u64,
                gauge.anvil_index_compaction_waiting_lanes = 0_u64,
                gauge.anvil_index_compaction_shared_workspace_bytes =
                    parallelism.shared_workspace_bytes() as u64,
                gauge.anvil_index_compaction_incremental_lane_workspace_bytes =
    ...
    fn emit_progress(&self, emission: CompactionEmission, heartbeat: bool) {
        ...
        let emit = || {
            ...
            tracing::info!(
                ...
                gauge.anvil_index_compaction_effective_lanes = lanes.count,
    ...
    fn emit_terminal(&self, emission: CompactionEmission, failed: bool) {
        ...
        tracing::info!(
            ...
            gauge.anvil_index_compaction_worker_limit = self.parallelism.worker_limit() as u64,
            ...
            histogram.anvil_index_compaction_duration_seconds = emission.elapsed_seconds,
...
mod tests {
    ...
    fn range_availability_caps_effective_lanes_and_wins_ties() {
        let parallelism = CompactionParallelism::new(
            4,
            anvil_index::compaction::COMPACTION_INCREMENTAL_LANE_WORKSPACE_BYTES,
        )
        .unwrap();
    ...
    fn fixed_lane_caps_have_a_deterministic_reason() {
        let ample_budget = (anvil_index::compaction::COMPACTION_SHARED_WORKSPACE_BYTES
            + 8 * anvil_index::compaction::COMPACTION_INCREMENTAL_LANE_WORKSPACE_BYTES)
            as u64;
        ...
        let budget = CompactionParallelism::for_budget(
            4,
            4,
            anvil_index::compaction::COMPACTION_SHARED_WORKSPACE_BYTES as u64,
        )
        .unwrap();
