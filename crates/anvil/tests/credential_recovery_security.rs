...
use anvil_store::{Store, StoreOptions, SystemBootstrapRequest};
use tempfile::TempDir;
use tonic::transport::{Channel, Endpoint};
use tonic::{Code, Request};

const SIGNING_KEY: &[u8] = b"anvil-credential-recovery-security-test-key";
...
async fn hostile_callers_fail_closed_and_system_recovery_is_exact_and_idempotent() {
    ...
    let mut acme_objects = anvil_storage::object_client(fixture.channel.clone(), &acme_token)
        .expect("owner token must form valid metadata");
    anvil_storage::put_chunks(
        &mut acme_objects,
        PutHeader {
...
fn administration(channel: &Channel, token: &str) -> anvil_storage::RawAdministrationClient {
    anvil_storage::administration_client(channel.clone(), token).unwrap()
}
...
async fn exchange(
    ...
    anvil_storage::exchange_client_credentials(
        channel.clone(),
        client_id.to_owned(),
