...
const CONTENT_TYPE_SHARD: &str = "application/vnd.anvil.osv+ndjson+zstd";
...
#[command(
    ...
    about = "Qualify Anvil 0.7 with a deterministic OSV shard workload"
)]
struct Args {
    /// URL of one clean Anvil 0.7 node, for example http://127.0.0.1:50051.
    #[arg(long)]
    endpoint: String,
...
async fn main() -> Result<()> {
    ...
    let report = QualificationReport {
        ...
        limitations: vec![
            ...
            "Shard attributes are carried by the authoritative manifest. Anvil 0.7 receives no per-object user metadata and this tool creates no metadata sidecar.",
...
fn snapshot_manifest(
    ...
    OsvSnapshotManifest {
        schema: "anvil.osv.snapshot-manifest.v1".into(),
        source_id: "osv".into(),
        ...
        format: "anvil.osv.source-record.ndjson.v1".into(),
