...
#[command(
    ...
    about = "Qualify Anvil 0.6 with a deterministic OSV shard workload"
)]
struct Args {
    /// URL of one clean Anvil 0.6 node, for example http://127.0.0.1:50051.
    #[arg(long)]
    endpoint: String,
...
async fn main() -> Result<()> {
    ...
    let report = QualificationReport {
        ...
        limitations: vec![
            ...
            "Shard attributes are carried by the authoritative manifest. Anvil 0.6 receives no per-object user metadata and this tool creates no metadata sidecar.",
