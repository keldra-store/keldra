...
async fn public_watch_hides_reserved_segments_and_advances_its_checkpoint() {
    ...
    for (path, command) in [
        ("_keldra", "reserved-root"),
        ("a/_keldra/meta.json", "reserved-descendant"),
        ("_keldraish", "ordinary-path"),
    ] {
        store
    ...
    assert_eq!(page.invalidations[0].key, key("_keldraish"));
}
...
async fn an_event_larger_than_the_journal_bound_rejects_the_whole_mutation() {
    ...
    assert!(matches!(
        &outcomes[0].result,
        Err(keldra_store::MutationError::SourceJournalRecordTooLarge { maximum: 1, .. })
    ));
...
async fn production_bulk_waits_for_source_journal_capacity_without_losing_the_write() {
    ...
    let waiting = {
        ...
        tokio::spawn(async move {
            store
                .bulk_write_with_backpressure(vec![keldra_store::BatchOperation::Put(put(
                    "second",
                    b"two",
                    "second-command",
                ))])
