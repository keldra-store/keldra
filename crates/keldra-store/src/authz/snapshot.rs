...
use keldra_authz::{Authorization, RealmId, Tuple};
...
const AUTHZ_REALM_TRANSFER_HASH_DOMAIN: &[u8] = b"keldra.authz-realm-transfer.v1\0";
...
impl AuthzRealmAggregate {
    ...
    fn validate_with_repository_limits(
        &self,
        evaluator_limits: keldra_authz::AuthorizationLimits,
    ...
    fn replace_realm(
            ...
            .is_none()
        {
            batch.put_cf(
                self.cf(CF_AUTHZ_SCHEMAS)?,
                schema_key,
                encode_json(&super::AuthzSchemaRevision::from(stored_schema.clone()))?,
            );
        }
        batch.put_cf(
            self.cf(CF_AUTHZ_SCHEMAS)?,
