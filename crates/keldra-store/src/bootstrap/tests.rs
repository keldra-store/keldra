use keldra_authz::{ObjectId, RealmId, TupleSubject};
...
async fn bootstrap_installs_one_complete_system_state_batch() {
    ...
    assert_eq!(tuple.object.id, ObjectId::Opaque("_keldra".into()));
    assert_eq!(
        tuple.subject,
...
async fn anonymous_subject_cannot_be_claimed_as_an_application() {
    ...
    let rejected = store.provision_tenant(ProvisionTenantRequest {
        storage_tenant: tenant("acme"),
        owner_app_id: keldra_authz::ANONYMOUS_SUBJECT_ID.into(),
