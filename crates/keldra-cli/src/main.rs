...
use keldra_storage::v1::{
    ...
    SetBucketVersioningRequest, TenantApplicationRole, TenantApplicationRoleTarget,
    UnlinkObjectRequest, WatchNow, WatchPrefixRequest, WatchRetainedBeginning, WatchStateHint,
};
...
use value_helpers::{
    address, clone_operation, lower_hex, parse_hex, present_head_line, put_operation,
    versioning_name,
};
...
struct Arguments {
    ...
    /// Mode-0600 file containing an exact short-lived access token, or `-` for piped stdin.
    #[arg(long, env = "KELDRA_ACCESS_TOKEN_FILE")]
    token_file: Option<PathBuf>,

    /// Mode-0600 JSON credential file generated by system bootstrap, or `-` for piped stdin.
    ...
    /// Mode-0600 file containing the exact client secret, or `-` for piped stdin.
    #[arg(long, env = "KELDRA_CLIENT_SECRET_FILE")]
...
enum Command {
    ...
    ProvisionTenant {
        ...
        /// Mode-0600 file containing the secret, or `-` for piped stdin.
        #[arg(long, env = "KELDRA_NEW_CLIENT_SECRET_FILE")]
        owner_client_secret_file: PathBuf,
    },
    ...
    /// Inspect the cluster's sole v1 protocol/storage capabilities.
...
async fn main() -> Result<()> {
    ...
    match arguments.command {
        ...
        Command::ProvisionTenant {
            ...
            owner_app_id,
            owner_client_id,
            owner_client_secret_file,
        } => {
            let owner_client_secret = load_client_secret_input(&owner_client_secret_file)?;
        ...
        Command::CreateApplication {
            ...
            let client_secret = load_client_secret_input(&client_secret_file)?;
...
fn print_cluster_capabilities(capabilities: &ClusterCapabilities) {
    println!(
        "active_protocol={} active_storage={} placement_term={} placement_index={}",
    ...
    physical_storage::print(
        capabilities.physical_storage.as_ref(),
        capabilities.logical_file_counts.as_ref(),
    );
}
...
async fn resolve_access_token(
    ...
    let credential_sources = usize::from(arguments.token_file.is_some())
        + usize::from(arguments.credentials_file.is_some())
        + usize::from(arguments.client_id.is_some() || arguments.client_secret_file.is_some());
    if credential_sources != 1 {
        bail!(
            "supply exactly one of --token-file, --credentials-file, or the --client-id/--client-secret-file pair"
        );
    }
    if let Some(token_file) = arguments.token_file.as_ref() {
        let token = load_client_secret_input(token_file)?;
        if token.is_empty() {
    ...
    let (client_id, client_secret) = match arguments.credentials_file.as_ref() {
        Some(path) => {
            let bytes = load_credential_input(path)?;
        ...
        None => {
            let client_id = arguments.client_id.clone().ok_or_else(|| {
                anyhow::anyhow!("--client-id and --client-secret-file are required together")
            })?;
            let client_secret_file = arguments.client_secret_file.as_ref().ok_or_else(|| {
                anyhow::anyhow!("--client-id and --client-secret-file are required together")
            })?;
            (client_id, load_client_secret_input(client_secret_file)?)
        }
    };
...
fn load_credential_input(path: &Path) -> Result<Vec<u8>> {
    if path != Path::new("-") {
        return load_credential_file(path);
    }
    let stdin = std::io::stdin();
    if stdin.is_terminal() {
        bail!("credential stdin must be redirected; interactive input could echo the secret");
    }
    let mut bytes = Vec::new();
    stdin
        .lock()
    ...
    if bytes.len() as u64 > MAX_CREDENTIAL_FILE_BYTES {
        bail!("credential input exceeds {MAX_CREDENTIAL_FILE_BYTES} bytes");
    }
    Ok(bytes)
}

fn load_client_secret_file(path: &Path) -> Result<String> {
    decode_exact_client_secret(load_credential_file(path)?)
}

fn load_client_secret_input(path: &Path) -> Result<String> {
    if path == Path::new("-") {
        return decode_exact_client_secret(load_credential_input(path)?);
    }
    load_client_secret_file(path)
}

fn decode_exact_client_secret(bytes: Vec<u8>) -> Result<String> {
    let secret = String::from_utf8(bytes).context("client secret must contain valid UTF-8")?;
    if secret.bytes().any(|byte| matches!(byte, b'\r' | b'\n')) {
        bail!("client secret must not contain CR or LF; provide the exact secret bytes");
    }
    Ok(secret)
}
...
mod tests {
    use super::{
        ...
        TenantRoleArgument, VersioningArgument, clone_operation, decode_bootstrap_credential,
        load_client_secret_file, load_credential_file, parse_hex, present_head_line, put_operation,
        versioning_name,
    };
    use clap::Parser as _;
    use keldra_storage::v1::{clone_object_request, put_header::Operation};
    #[cfg(unix)]
    ...
    fn clone_commands_preserve_addresses_version_and_condition() {
        let clone = Arguments::try_parse_from(
            "keldra --token-file /run/secrets/token clone-object acme objects source 9 copy --command-id clone-1 --durability replicated --if-version 4"
                .split_whitespace(),
        ).unwrap();
        ...
        assert!(
            Arguments::try_parse_from(
                "keldra --token-file /run/secrets/token clone-object a b s 1 d --command-id c --if-absent --if-version 2"
                    .split_whitespace(),
            )
            .is_err()
        );
        ...
        let link = Arguments::try_parse_from(
            "keldra --token-file /run/secrets/token link-object acme objects alias target --command-id link-1"
                .split_whitespace(),
        )
        .unwrap();
        ...
        let unlink = Arguments::try_parse_from(
            "keldra --token-file /run/secrets/token unlink-object acme objects alias --command-id unlink-1"
                .split_whitespace(),
        )
        .unwrap();
    ...
    fn version_management_commands_are_explicit() {
        let default_create = Arguments::try_parse_from([
            "keldra",
            "--token-file",
            "/run/secrets/token",
            "create-bucket",
    ...
    #[test]
    fn credential_values_are_not_accepted_in_process_arguments() {
        assert!(
            Arguments::try_parse_from([
                "keldra",
                "--token",
