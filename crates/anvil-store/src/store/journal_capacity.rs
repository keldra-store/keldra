...
mod tests {
    ...
    async fn publish_progress(store: &Store, command: &str) {
        ...
        store
            .mutate_derived_progress_with_governance_and_backpressure(
                PublishRequest {
                    key: ObjectKey::new("tenant", "bucket", "_anvil/indexes/v3/7/current").unwrap(),
                    blob,
    ...
    async fn derived_publication_can_repay_a_full_journal_and_wake_a_source_writer() {
        ...
        let forged = store
            .bulk_write(vec![put("_anvil/indexes/v3/7/current", "forged-progress")])
            .await;
        ...
        assert!(
            store
                .head(&ObjectKey::new("tenant", "bucket", "_anvil/indexes/v3/7/current").unwrap())
                .unwrap()
                .is_some()
        );
