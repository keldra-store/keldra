...
pub fn definition_path(name: &str) -> Result<String, IndexError> {
    ...
    Ok(format!("_keldra/indices/v4/definitions/{name}"))
}

pub fn artifact_path(index_id: u64, hash: [u8; 32]) -> String {
    format!("_keldra/indices/v4/{index_id}/artifacts/{}", hex(hash))
}

pub fn manifest_path(index_id: u64, hash: [u8; 32]) -> String {
    format!("_keldra/indices/v4/{index_id}/manifests/{}", hex(hash))
}
...
mod tests {
    ...
    fn reserved_paths_are_canonical() {
        assert_eq!(
            ...
            "_keldra/indices/v4/definitions/search"
        );
        assert!(definition_path("a/b").is_err());
        assert_eq!(current_path(12), "_keldra/indices/v4/12/current");
