    ...
    fn validate(&self) -> Result<(), IndexError> {
            ...
            || self.version == 0
        {
            return Err(IndexError::InvalidDefinition(
                "v1 object identity is invalid".into(),
            ));
        }
        Ok(())
    }
...
const QUERY_CACHE_IDENTITY_PREFIX: &str = "_keldra/index-cache/v1/";
...
fn decode_hex(value: u8) -> Result<u8, IndexError> {
    match value {
        ...
        _ => Err(IndexError::InvalidFormat(
            "format-v1 query-cache identity hex",
        )),
    }
}
...
impl StableDocumentKey {
    ...
    /// Query admission resolves the stable key through the pinned format-v1
    ...
    pub fn from_query_cache_identity(identity: &ObjectIdentity) -> Result<Self, IndexError> {
        let encoded = identity
            ...
            .ok_or(IndexError::InvalidFormat(
                "format-v1 query-cache identity prefix",
            ))?;
        if identity.version == 0 || encoded.len() != 64 {
            return Err(IndexError::InvalidFormat(
                "format-v1 query-cache identity shape",
            ));
        }
...
impl ProjectedDocumentState {
    ...
    pub fn resident_bytes(&self) -> Result<usize, IndexError> {
        std::mem::size_of::<Self>()
            .checked_add(self.owned_heap_bytes()?)
            .ok_or(IndexError::OffsetOverflow)
    }

    pub(crate) fn owned_heap_bytes(&self) -> Result<usize, IndexError> {
        let mut bytes = self.head.source_path.capacity();
