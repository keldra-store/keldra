//! Immutable, bounded page tree for v1 query-run descriptors.
//!
...
fn validate_reference(r: QueryRunReference) -> Result<(), IndexError> {
    if r.hash == [0; 32]
        || r.encoded_bytes == 0
        || r.sequence == 0
        || r.source_start_offset >= r.next_offset
    {
...
fn put_reference(out: &mut Vec<u8>, r: QueryRunReference) {
    ...
    out.extend_from_slice(&r.encoded_bytes.to_be_bytes());
