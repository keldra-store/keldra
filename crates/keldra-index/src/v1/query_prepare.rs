...
fn validate_run_identity(
    ...
    if catalog == [0; 32] || sequence == 0 || source_start_offset >= next_offset {
        return Err(IndexError::InvalidDefinition(
            "v1 query run identity is invalid".into(),
        ));
    }
    Ok(())
}
...
fn merge_term_delta(
    ...
    {
        return Err(IndexError::InvalidDefinition(
            "v1 query preparation contains conflicting term/document deltas".into(),
        ));
    }
...
fn encode_term_shards(
    ...
    for delta in documents {
        ...
        if delta.live && !delta.positions.is_empty() {
            ...
            pending_position_records = pending_position_records
                .checked_add(1)
                .ok_or(IndexError::OffsetOverflow)?;
        }
...
pub(super) fn encode_term_shard(
    recipe: RecipeIdentity,
...
fn insert_unique(
    ...
    if records.insert(record.key.clone(), record).is_some() {
        return Err(IndexError::InvalidDefinition(
            "v1 query preparation contains duplicate record keys".into(),
        ));
    }
    Ok(())
}
...
fn reject_duplicate_records(records: &[QueryBlockRecord]) -> Result<(), IndexError> {
    if records.windows(2).any(|pair| pair[0].key >= pair[1].key) {
        return Err(IndexError::InvalidDefinition(
            "v1 query preparation records are not unique canonical order".into(),
        ));
    }
    Ok(())
}
