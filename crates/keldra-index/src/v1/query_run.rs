//! Semantic oracle for the eventual format-v1 Typed JSON query mini-runs.
//!
...
fn index_state(
    ...
    if field.contract.field_type == FieldType::Text {
        for (term, positions) in super::query_blocks::analyzed_terms(Some(state))? {
            field.text.entry(term).or_default().insert(key, positions);
        }
    }
    Ok(())
}
