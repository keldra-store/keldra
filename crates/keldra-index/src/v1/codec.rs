...
const DIRECTORY_MAGIC: &[u8; 8] = b"K1CDIR01";
const GENERATION_MAGIC: &[u8; 8] = b"K1PGEN01";
const EMPTY_COMPONENT_DIRECTORY_DOMAIN: &[u8] = b"keldra.index.v1.empty-component-directory/v1";
const CURRENT_MAGIC: &[u8; 8] = b"K1PCUR01";
const BINDING_MAGIC: &[u8; 8] = b"K1BIND01";
...
pub fn decode_projection_current(bytes: &[u8]) -> Result<ProjectionCurrent, IndexError> {
    ...
    current.partition.validate()?;
    if current.generation_hash == [0; 32] || current.generation_revision == 0 {
        return Err(IndexError::InvalidFormat("projection current identity"));
    }
    Ok(current)
}
...
fn put_query_stream_root(out: &mut Vec<u8>, root: ProjectionQueryStreamRoot) {
    out.extend_from_slice(&root.stream_root_hash);
    put_u64(out, root.stream_root_encoded_bytes);
...
impl<'a> Decoder<'a> {
    ...
    fn query_stream_root(&mut self) -> Result<ProjectionQueryStreamRoot, IndexError> {
        Ok(ProjectionQueryStreamRoot {
            ...
            stream_root_encoded_bytes: self.u64()?,
            run_count: self.u64()?,
...
mod tests {
    ...
    fn current_pointer_round_trips_and_binds_one_exact_generation() {
        ...
        corrupt[20] ^= 1;
        let structurally_valid = decode_projection_current(&corrupt).unwrap();
        assert_ne!(structurally_valid, current);
        assert!(structurally_valid.validate_against(&generation).is_err());
    }
