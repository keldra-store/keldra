...
const PAGE_MAGIC: &[u8; 8] = b"K1CSTR01";
    ...
    actual.first_sequence == expected.first_sequence
        ...
        && (!expected.summary_exact
            || (actual.minimum_key == expected.minimum_key
                ...
                && actual.level_counts == expected.level_counts))
}
...
pub fn resolve_component_record_from_verified_artifacts(
    ...
    for descriptor in segments.iter().rev().filter(|run| {
...
pub fn lookup_component_record_in_verified_pack(
    ...
    let selected = cursor
        .seek_to(stable_key)?
        .filter(|record| record.stable_key == stable_key)
...
pub fn compact_component_runs(
    ...
    let mut heads = cursors
        ...
        .collect::<Result<Vec<_>, _>>()?;
    let mut pending_keys = BTreeMap::<StableDocumentKey, Vec<usize>>::new();
    for (index, record) in heads.iter().enumerate() {
    ...
    while let Some((key, lanes)) = pending_keys.pop_first() {
        ...
        for index in lanes {
            ...
            if let Some(record) = heads[index] {
                pending_keys
                    .entry(record.stable_key)
                    .or_default()
                    .push(index);
...
impl Child {
    fn from_segments(
        ...
        let mut level_counts = BTreeMap::<u8, u64>::new();
        for segment in segments {
            let count = level_counts.entry(segment.level).or_default();
    ...
    fn from_children(
        ...
        let mut level_counts = BTreeMap::<u8, u64>::new();
        for child in children {
            for &(level, count) in &child.level_counts {
                let total = level_counts.entry(level).or_default();
                *total = total.checked_add(count).ok_or(IndexError::OffsetOverflow)?;
            }
        }
        Ok(Self {
    ...
    match page {
        ...
        Page::Branch(children) => {
            ...
            for child in children {
                ...
                bytes.push(child.level_counts.len() as u8);
                for (level, count) in &child.level_counts {
                    bytes.push(*level);
            ...
            for _ in 0..count {
                children.push(Child {
                    ...
                    level_counts: {
                        ...
                        for _ in 0..count {
                            counts.push((input.byte()?, input.u64()?));
                        }
                        counts
                    },
                    summary_exact: true,
        ...
        || children.iter().any(|child| {
            child.first_sequence == 0
                ...
                || child
                    ...
                    != Some(child.segment_count)
                || child
                    .level_counts
                    .iter()
                    .any(|(level, count)| *level > 63 || *count == 0)
                || child
                    .level_counts
                    .windows(2)
