...
pub const MAX_CATALOG_LINEAGE_GENERATIONS: usize = 4_096;
pub const MAX_CATALOG_RANGES_PER_RECIPE: usize = 2_048;

use super::RecipeIdentity;

#[derive(Clone, Copy, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub struct CatalogOrdinalRange {
    pub first: u32,
    pub last: u32,
}

/// Recipe-scoped ranges into one activation-wide catalog lineage. Shared
/// generation hashes are stored once regardless of recipe count.
#[derive(Clone, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub struct QueryRecipeCatalogProof {
    pub recipe: RecipeIdentity,
    pub accepted_ordinal_ranges: Vec<CatalogOrdinalRange>,
}

impl QueryRecipeCatalogProof {
    pub fn validate(&self, lineage_len: usize, active_ordinal: u32) -> Result<(), IndexError> {
        if lineage_len == 0
            || lineage_len > MAX_CATALOG_LINEAGE_GENERATIONS
            || self.accepted_ordinal_ranges.is_empty()
            || self.accepted_ordinal_ranges.len() > MAX_CATALOG_RANGES_PER_RECIPE
            || self
                .accepted_ordinal_ranges
                .iter()
                .any(|range| range.first > range.last || range.last as usize >= lineage_len)
            || self.accepted_ordinal_ranges.windows(2).any(|ranges| {
                ranges[0]
                    .last
                    .checked_add(1)
                    .is_none_or(|next| next >= ranges[1].first)
            })
            || !self.accepts_ordinal(active_ordinal)
        {
...
impl ProjectionCatalogTransition {
    ...
    pub fn catalog_lineage_and_recipe_proofs(
        &self,
        previous_catalog: [u8; 32],
        previous_lineage: &[[u8; 32]],
        previous_proofs: &[QueryRecipeCatalogProof],
        active_recipes: &[RecipeIdentity],
        next_catalog: [u8; 32],
    ) -> Result<(Vec<[u8; 32]>, Vec<QueryRecipeCatalogProof>), IndexError> {
        if previous_catalog == [0; 32]
            || next_catalog == [0; 32]
            || previous_catalog == next_catalog
            || previous_lineage.is_empty()
            || previous_lineage.len() >= MAX_CATALOG_LINEAGE_GENERATIONS
            || previous_lineage.last() != Some(&previous_catalog)
            || previous_lineage
                .iter()
        ...
        let previous_active =
            u32::try_from(previous_lineage.len() - 1).map_err(|_| IndexError::OffsetOverflow)?;
        for proof in previous_proofs {
            proof.validate(previous_lineage.len(), previous_active)?;
        }
        let previous = previous_proofs
            .iter()
            .map(|proof| (proof.recipe, proof))
            .collect::<BTreeMap<_, _>>();
        let mut lineage = previous_lineage.to_vec();
        lineage.push(next_catalog);
        let active_ordinal =
            u32::try_from(lineage.len() - 1).map_err(|_| IndexError::OffsetOverflow)?;
        let proofs = active_recipes
            .iter()
            .map(|recipe| {
                let mut accepted_ordinal_ranges =
                ...
                if let Some(last) = accepted_ordinal_ranges.last_mut()
                    && last.last.checked_add(1) == Some(active_ordinal)
                {
                    last.last = active_ordinal;
                } else {
                    accepted_ordinal_ranges.push(CatalogOrdinalRange {
                        first: active_ordinal,
                        last: active_ordinal,
                    });
                }
                ...
                proof.validate(lineage.len(), active_ordinal)?;
                Ok(proof)
            })
...
pub struct ProjectionQueryStreamRoot {
    pub stream_root_hash: [u8; 32],
    pub stream_root_encoded_bytes: u64,
    pub run_count: u64,
    pub first_sequence: u64,
    pub last_sequence: u64,
...
impl ProjectionQueryStreamRoot {
    pub fn empty(
        ...
        hasher.update(b"keldra.index.v1.empty-query-stream/v1\0");
    ...
    pub fn validate_at(
        ...
        if self.stream_root_hash == [0; 32]
            ...
            || (empty
                && (self.stream_root_encoded_bytes != 0
                    || self.first_sequence != 0
...
mod tests {
    ...
    fn catalog_transition_reuses_only_retained_recipes_and_builds_exact_proofs() {
        ...
        let (lineage, proofs) = transition
            .catalog_lineage_and_recipe_proofs(
                [7; 32],
                &[[6; 32], [7; 32]],
            ...
            .unwrap();
        assert_eq!(lineage, vec![[6; 32], [7; 32], [8; 32]]);
        assert_eq!(
            proofs,
            vec![
                QueryRecipeCatalogProof {
                    recipe: recipe(4),
                    accepted_ordinal_ranges: vec![CatalogOrdinalRange { first: 0, last: 2 }],
                },
                QueryRecipeCatalogProof {
                    recipe: recipe(6),
                    accepted_ordinal_ranges: vec![CatalogOrdinalRange { first: 2, last: 2 }],
                },
            ]
        );
    }
