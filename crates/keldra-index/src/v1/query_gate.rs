...
pub fn encode_document_gate(gate: QueryDocumentGate) -> Result<QueryBlockRecord, IndexError> {
        ...
        || !valid_identity(&gate)
    {
        return Err(IndexError::InvalidDefinition(
            "v1 gate version or identity is invalid".into(),
        ));
    }
...
pub fn decode_document_gate(
    ...
    if !valid_identity(&gate) {
        return Err(IndexError::InvalidFormat("v1 document gate identity"));
    }
    Ok(gate)
}
...
fn read_u64(bytes: &[u8], offset: usize) -> Result<u64, IndexError> {
    Ok(u64::from_be_bytes(
        bytes
            ...
            .ok_or(IndexError::InvalidFormat("v1 document gate"))?
            .try_into()
...
fn decode_path(bytes: &[u8]) -> Result<Option<String>, IndexError> {
    ...
    let path = std::str::from_utf8(bytes)
        .map_err(|_| IndexError::InvalidFormat("v1 document gate path"))?;
    if path.contains('\0') {
        return Err(IndexError::InvalidFormat("v1 document gate path"));
    }
