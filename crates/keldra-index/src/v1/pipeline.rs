...
/// Exact byte credits for retained v1 pipeline allocations.
///
...
impl PreparedProjectionRow {
    ...
    fn owned_heap_bytes(&self) -> Result<usize, IndexError> {
        let mut bytes = self
            .source_path
            .capacity()
            .checked_add(
        ...
        for state in &self.projected_states {
            bytes = bytes
                .checked_add(state.owned_heap_bytes()?)
                .ok_or(IndexError::OffsetOverflow)?;
        }
        Ok(bytes)
    }
    ...
    fn new(
        ...
        for row in &rows {
            ...
            resident_bytes = resident_bytes
                .checked_add(row.owned_heap_bytes()?)
                .ok_or(IndexError::OffsetOverflow)?;
        }
...
mod tests {
    ...
    #[test]
    fn batch_uses_changed_key_overlay_without_cloning_accumulated_history() {
