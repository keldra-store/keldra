...
fn validate_delta(delta: &SealedComponentDelta) -> Result<(), IndexError> {
    // A SealedComponentDelta is created inside this process from validated,
    // typed records. Its content hash is its addressing identity; reparsing and
    // hashing the freshly-created bytes here proved the same work a second time.
    if delta.encoded_bytes != delta.bytes.len() as u64
        || delta.records == 0
        || delta.minimum_key > delta.maximum_key
        || delta.hash == [0; 32]
    {
