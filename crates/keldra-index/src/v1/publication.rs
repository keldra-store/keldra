...
struct PreparedProjectionComponents {
    packs: Vec<SealedProjectionDeltaPack>,
...
fn prepare_projection_components(
    ...
    Ok(PreparedProjectionComponents {
        packs,
        stream_pages,
        generation,
        pack_credits,
...
fn finalize_projection_components(
    components: PreparedProjectionComponents,
) -> Result<PreparedProjectionGeneration, IndexError> {
    let encoded_generation = encode_projection_generation(&components.generation)?;
    ...
    let prepared = PreparedProjectionGeneration {
        ...
        current,
        _pack_credits: components.pack_credits,
    };
...
mod tests {
    ...
    fn initial_publication_contains_every_artifact_before_one_complete_generation() {
        ...
        let current = crate::v1::decode_projection_current(&prepared.current).unwrap();
    ...
    fn atomic_prepare_binds_components_query_run_generation_and_current_to_one_cut() {
        ...
        let current = crate::v1::decode_projection_current(&first.current).unwrap();
        current.validate_against(&generation).unwrap();
