...
use super::{
    CatalogOrdinalRange, MAX_CATALOG_LINEAGE_GENERATIONS, MAX_CATALOG_RANGES_PER_RECIPE,
    MAX_QUERY_RECIPE_CATALOG_PROOFS, ProjectionGenerationReference, ProjectionPartitionIdentity,
    QueryRecipeCatalogProof,
};
use crate::IndexError;

const DIRECTORY_MAGIC: &[u8; 8] = b"K1FDIR01";
const ACTIVATION_MAGIC: &[u8; 8] = b"K1CACT01";
const FORMAT: u16 = 1;
...
pub struct ProjectionCatalogActivation {
    ...
    /// Per-recipe ordinal ranges into `catalog_lineage`.
    pub recipe_catalog_proofs: Vec<QueryRecipeCatalogProof>,
}

impl ProjectionCatalogActivation {
    pub fn validate(&self) -> Result<(), IndexError> {
        let active_ordinal = self.catalog_lineage.len().checked_sub(1).ok_or_else(|| {
            IndexError::InvalidDefinition("projection catalog lineage is empty".into())
        })?;
        let active_ordinal =
            u32::try_from(active_ordinal).map_err(|_| IndexError::OffsetOverflow)?;
        if self.family_id == [0; 32]
            || self.physical_catalog_generation == [0; 32]
            || self.definition_catalog_hash == [0; 32]
            || self.recipe_catalog_hash == [0; 32]
            || self.catalog_lineage.len() > MAX_CATALOG_LINEAGE_GENERATIONS
            || self.catalog_lineage.last() != Some(&self.physical_catalog_generation)
            || self
                .catalog_lineage
                .iter()
        ...
        for proof in &self.recipe_catalog_proofs {
            proof.validate(self.catalog_lineage.len(), active_ordinal)?;
        }
        if self
            .baseline
            .iter()
...
pub fn encode_projection_catalog_activation(
    ...
    u32_(&mut out, v.catalog_lineage.len() as u32);
    for generation in &v.catalog_lineage {
    ...
    for proof in &v.recipe_catalog_proofs {
        ...
        u32_(&mut out, proof.accepted_ordinal_ranges.len() as u32);
        for range in &proof.accepted_ordinal_ranges {
            u32_(&mut out, range.first);
            u32_(&mut out, range.last);
        }
    }
    Ok(out)
}
...
pub fn decode_projection_catalog_activation(
    ...
    let lineage_count = d.count()?;
    if lineage_count == 0 || lineage_count > MAX_CATALOG_LINEAGE_GENERATIONS {
        return invalid("query catalog lineage generation count exceeds limit");
    }
    let mut catalog_lineage = Vec::with_capacity(lineage_count);
    for _ in 0..lineage_count {
        catalog_lineage.push(d.a32()?);
    }
    let n = d.count()?;
    ...
    for _ in 0..n {
        let recipe = crate::v1::RecipeIdentity::new(d.a32()?)?;
        let ranges = d.count()?;
        if ranges > MAX_CATALOG_RANGES_PER_RECIPE {
            return invalid("query recipe catalog proof range count exceeds limit");
        }
        let mut accepted_ordinal_ranges = Vec::with_capacity(ranges);
        for _ in 0..ranges {
            accepted_ordinal_ranges.push(CatalogOrdinalRange {
                first: d.u32()?,
                last: d.u32()?,
            });
        }
...
mod tests {
    ...
    fn activation_requires_every_partition_to_cover_its_baseline() {
        ...
        let activation = ProjectionCatalogActivation {
            ...
            recipe_catalog_proofs: vec![QueryRecipeCatalogProof {
                recipe: crate::v1::RecipeIdentity::new([9; 32]).unwrap(),
    ...
    fn activation_round_trips_retained_and_new_recipe_catalog_proofs() {
        let activation = ProjectionCatalogActivation {
            ...
            recipe_catalog_proofs: vec![
                QueryRecipeCatalogProof {
                    recipe: crate::v1::RecipeIdentity::new([8; 32]).unwrap(),
                    accepted_ordinal_ranges: vec![CatalogOrdinalRange { first: 0, last: 1 }],
                },
                QueryRecipeCatalogProof {
                    recipe: crate::v1::RecipeIdentity::new([9; 32]).unwrap(),
                    accepted_ordinal_ranges: vec![CatalogOrdinalRange { first: 1, last: 1 }],
                },
            ],
        };
        let encoded = encode_projection_catalog_activation(&activation).unwrap();
    ...
    #[test]
    fn activation_serializes_shared_catalog_lineage_once() {
        let mut catalog_lineage = (0_u64..100)
            .map(|ordinal| {
                let mut generation = [0_u8; 32];
                generation[..8].copy_from_slice(&(ordinal + 1).to_le_bytes());
                generation
            })
            .collect::<Vec<_>>();
        *catalog_lineage.last_mut().unwrap() = [2; 32];
        let recipe_catalog_proofs = (0_u16..256)
            .map(|ordinal| {
                let mut identity = [0_u8; 32];
                identity[..2].copy_from_slice(&(ordinal + 1).to_be_bytes());
                QueryRecipeCatalogProof {
                    recipe: crate::v1::RecipeIdentity::new(identity).unwrap(),
                    accepted_ordinal_ranges: vec![CatalogOrdinalRange { first: 0, last: 99 }],
                }
            })
            .collect();
        let activation = ProjectionCatalogActivation {
        ...
        let encoded = encode_projection_catalog_activation(&activation).unwrap();
        assert!(encoded.len() < 20 * 1024, "encoded {} bytes", encoded.len());
        assert_eq!(
            decode_projection_catalog_activation(&encoded).unwrap(),
            activation
        );
