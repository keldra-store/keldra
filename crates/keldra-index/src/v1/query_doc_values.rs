...
pub fn encode_doc_value(
    ...
    if value.material_source_version == 0 {
        return Err(IndexError::InvalidDefinition(
            "v1 doc value version is zero".into(),
        ));
    }
    let mut encoded = value.material_source_version.to_be_bytes().to_vec();
    match &value.value {
        Some(values) => {
            validate_value_order(values, limits, true)?;
            encoded.push(1);
            put_u32(&mut encoded, values.len())?;
            for value in values {
                let scalar = encode_scalar_sort_key(value)?;
                if scalar.len() > limits.maximum_key_bytes {
                    return resource(scalar.len(), limits.maximum_key_bytes);
                }
...
pub fn decode_doc_value(
    ...
    let document = StableDocumentKey::from_bytes(
        record
            .key
            .try_into()
            .map_err(|_| IndexError::InvalidFormat("v1 doc value key"))?,
    )?;
    let material_source_version = read_u64(record.value, 0)?;
    if material_source_version == 0 {
        return Err(IndexError::InvalidFormat("v1 doc value version"));
    }
    let value = match record.value.get(8) {
        ...
        Some(1) => {
            ...
            if count > limits.maximum_records || count > record.value.len().saturating_sub(13) / 4 {
                return Err(IndexError::InvalidFormat("v1 doc value count is unbounded"));
            }
            ...
            for _ in 0..count {
                ...
                if used != encoded.len() {
                    return Err(IndexError::InvalidFormat("v1 doc value scalar"));
                }
                values.push(value);
                offset = end;
            }
            if offset != record.value.len() {
                return Err(IndexError::InvalidFormat("v1 doc value trailing bytes"));
            }
            validate_value_order(&values, limits, false)?;
            Some(values)
        }
        _ => return Err(IndexError::InvalidFormat("v1 doc value")),
    };
    Ok(QueryDocValue {
        document,
...
fn validate_value_order(
    ...
    if values.len() > limits.maximum_records {
        return if definition {
            ...
            Err(IndexError::InvalidFormat("v1 doc value count is unbounded"))
        };
    }
    if values.windows(2).any(|pair| pair[0] > pair[1]) {
        return Err(if definition {
            IndexError::InvalidDefinition("v1 doc values are noncanonical".into())
        } else {
            IndexError::InvalidFormat("v1 doc values are noncanonical")
        });
    }
    Ok(())
}
