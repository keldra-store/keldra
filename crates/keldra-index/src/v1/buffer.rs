...
use super::{
    ...
    ProjectedDocumentState, RecipeIdentity, StableDocumentKey,
};

const SEGMENT_MAGIC: &[u8; 8] = b"K1DELTA1";
const SEGMENT_FORMAT: u16 = 1;
const SEGMENT_RESTART_INTERVAL: usize = 64;
...
pub struct BorrowedComponentDeltaRecord<'a> {
    pub stable_key: StableDocumentKey,
    pub replacement: Option<&'a [u8]>,
}

/// Integrity-checked, allocation-free cursor over one sorted component run.
/// The encoded run remains borrowed by the caller for the cursor lifetime.
...
impl Clone for ProjectionMutationBuffer {
    fn clone(&self) -> Self {
        #[cfg(test)]
...
impl ProjectionMutationBuffer {
    ...
    /// Admission for one complete replacement, without cloning or encoding it.
    /// Charges component/entry overhead independently, hence remains an upper
    /// bound when several rows share a component or overwrite the same key.
    pub fn source_replacement_admission_bytes(
        path: &str,
        states: &[ProjectedDocumentState],
    ) -> Result<usize, IndexError> {
        let overhead = COMPONENT_ACCOUNTING_BYTES + ENTRY_ACCOUNTING_BYTES;
        let mut bytes = overhead
            .checked_add(if states.is_empty() {
                0
            } else {
                40usize
        ...
        if states.is_empty() {
            return bytes
                ...
                .and_then(|bytes| bytes.checked_add(26 + path.len()))
                .ok_or(IndexError::OffsetOverflow);
        }
        for state in states {
            let head = 26usize
                .checked_add(state.head.source_path.len())
                .and_then(|bytes| {
                    bytes.checked_add(
                        state
                            .head
            ...
            bytes = bytes
                .checked_add(overhead)
                .and_then(|bytes| bytes.checked_add(head))
                .ok_or(IndexError::OffsetOverflow)?;
            for value in state.memberships.iter().chain(&state.fields) {
                bytes = bytes
                    .checked_add(overhead)
    ...
    /// Atomically merge a batch-local changed-key overlay. The sizing pass is
    /// completed before either buffer is mutated, so admission failure leaves
    /// the accumulated projection untouched without cloning its cold history.
    pub(crate) fn merge_overlay(&mut self, overlay: Self) -> Result<(), IndexError> {
        let mut merged_bytes = self.used_bytes;
        for (component, entries) in &overlay.components {
            let existing = self.components.get(component);
            if existing.is_none() {
            ...
            for (key, replacement) in entries {
                if let Some(old) = existing.and_then(|entries| entries.get(key)) {
                    merged_bytes = merged_bytes
                        .checked_sub(accounted_record_bytes(old))
        ...
        if merged_bytes > self.limit_bytes {
            return Err(IndexError::ResourceLimit {
                needed: merged_bytes,
        ...
        self.used_bytes = merged_bytes;
        Ok(())
    }

    /// Emit a complete replacement for one indexed document.
    pub fn apply_state(&mut self, state: &ProjectedDocumentState) -> Result<(), IndexError> {
        state.validate()?;
        if state.head.source_record != 0
            || state.head.material_source_version != state.head.source_version
        {
            return Err(IndexError::InvalidDefinition(
                "replacement requires record zero and exact material version".into(),
            ));
        }
        let delta = state.replacement()?;
        self.apply_delta(state.head.stable_key, delta, Vec::new())
    }

    /// Apply one complete Typed JSON document or an authoritative deletion.
    /// Expanded records are not supported by this replacement format.
    pub fn apply_source_states(
        ...
        working.apply_source_states_in_place(source_scope, source_path, source_version, current)?;
        *self = working;
        Ok(())
    }

    pub(crate) fn apply_source_states_in_place(
        &mut self,
        source_scope: [u8; 32],
        source_path: &str,
        source_version: u64,
        ...
        if source_scope == [0; 32] || source_version == 0 || current.len() > 1 {
            return Err(IndexError::InvalidDefinition(
                "replacement source requires one Typed JSON record".into(),
            ));
        }
        let current =
...
pub(super) fn seal_component(
    ...
    let minimum_key = *records.first_key_value().expect("nonempty component").0;
    let maximum_key = *records.last_key_value().expect("nonempty component").0;
    let record_count = records.len() as u64;
    let mut bytes = Vec::new();
    bytes.extend_from_slice(SEGMENT_MAGIC);
    put_u16(&mut bytes, SEGMENT_FORMAT);
    put_component(&mut bytes, component);
    put_u64(&mut bytes, record_count);
    put_u32(&mut bytes, SEGMENT_RESTART_INTERVAL as u32);
    let restart_count = records.len().div_ceil(SEGMENT_RESTART_INTERVAL);
    put_u32(&mut bytes, restart_count as u32);
    let restart_offsets_start = bytes.len();
    bytes.resize(restart_offsets_start + restart_count * 4, 0);
    let records_start = bytes.len();
    let mut logical_bytes = 0_u64;
    for (index, (key, replacement)) in records.into_iter().enumerate() {
        if index % SEGMENT_RESTART_INTERVAL == 0 {
            let offset = u32::try_from(bytes.len() - records_start)
                .map_err(|_| IndexError::OffsetOverflow)?;
            let at = restart_offsets_start + (index / SEGMENT_RESTART_INTERVAL) * 4;
...
pub fn decode_component_delta_segment(bytes: &[u8]) -> Result<DecodedComponentDelta, IndexError> {
    ...
    let restart_interval = input.u32()? as usize;
    let restart_count = input.u32()? as usize;
    if restart_interval != SEGMENT_RESTART_INTERVAL
        || restart_count != count.div_ceil(SEGMENT_RESTART_INTERVAL)
    {
        return Err(IndexError::InvalidFormat(
            "projection delta restart table is invalid",
        ));
    }
    let mut restart_offsets = Vec::with_capacity(restart_count);
    for _ in 0..restart_count {
        restart_offsets.push(input.u32()? as usize);
    }
    ...
    for index in 0..count {
        if index % SEGMENT_RESTART_INTERVAL == 0
            && restart_offsets[index / SEGMENT_RESTART_INTERVAL]
                != input.offset.saturating_sub(records_start)
        {
            return Err(IndexError::InvalidFormat(
                "projection delta restart offset is invalid",
            ));
        }
...
impl<'a> ComponentDeltaCursor<'a> {
    pub fn new(bytes: &'a [u8]) -> Result<Self, IndexError> {
        let mut input = Decoder::new(bytes);
        input.expect(SEGMENT_MAGIC)?;
        if input.u16()? != SEGMENT_FORMAT {
            return Err(IndexError::InvalidFormat(
        ...
        let restart_count = input.u32()? as usize;
        if restart_interval != SEGMENT_RESTART_INTERVAL
            || restart_count != remaining.div_ceil(SEGMENT_RESTART_INTERVAL)
        {
            return Err(IndexError::InvalidFormat(
                "projection delta restart table is invalid",
            ));
        ...
        for restart in 0..restart_count {
            let at = restart_offsets_start + restart * 4;
            ...
            if (restart == 0 && offset != 0)
                ...
                || offset >= input.bytes.len().saturating_sub(records_start)
            {
                return Err(IndexError::InvalidFormat(
                    "projection delta restart offset is invalid",
                ));
    ...
    pub fn seek_to(
        &mut self,
        target: StableDocumentKey,
    ) -> Result<Option<BorrowedComponentDeltaRecord<'a>>, IndexError> {
        let mut low = 0usize;
        let mut high = self.restart_count;
        while low < high {
            let middle = (low + high) / 2;
            let offset_at = self.restart_offsets_start + middle * 4;
            let offset = u32::from_le_bytes(
                self.input.bytes[offset_at..offset_at + 4]
                    .try_into()
                    .map_err(|_| IndexError::Integrity)?,
            ) as usize;
            let key_at = self
                ...
                .checked_add(offset)
                .ok_or(IndexError::OffsetOverflow)?;
            let key = StableDocumentKey::from_bytes(
                self.input
                    .bytes
                    .get(key_at..key_at + 32)
                    .ok_or(IndexError::Integrity)?
                    .try_into()
                    .map_err(|_| IndexError::Integrity)?,
            )?;
        ...
        let restart = low.saturating_sub(1);
        let offset_at = self.restart_offsets_start + restart * 4;
        let offset = u32::from_le_bytes(
            self.input.bytes[offset_at..offset_at + 4]
                .try_into()
                .map_err(|_| IndexError::Integrity)?,
        ...
        self.input.offset = self
            ...
            .checked_add(offset)
            .ok_or(IndexError::OffsetOverflow)?;
        let consumed = restart.saturating_mul(SEGMENT_RESTART_INTERVAL);
        self.remaining = usize::try_from(self.records)
            .map_err(|_| IndexError::OffsetOverflow)?
            .saturating_sub(consumed);
        self.previous = None;
        while let Some(record) = self.next_record()? {
            if record.stable_key >= target {
    ...
    pub fn next_record(&mut self) -> Result<Option<BorrowedComponentDeltaRecord<'a>>, IndexError> {
        if self.remaining == 0 {
            self.input.finish_ref()?;
            return Ok(None);
        }
        let stable_key = StableDocumentKey::from_bytes(self.input.array_32()?)?;
        if self.previous.is_some_and(|previous| previous >= stable_key) {
            return Err(IndexError::UnsortedRecords);
        }
        let replacement = match self.input.byte()? {
            0 => None,
            1 => {
                let length =
                    usize::try_from(self.input.u64()?).map_err(|_| IndexError::OffsetOverflow)?;
                Some(self.input.take(length)?)
            }
            _ => return Err(IndexError::Decode("delta presence is invalid".into())),
        };
        self.previous = Some(stable_key);
        self.remaining -= 1;
        if self.remaining == 0 {
            self.input.finish_ref()?;
        }
        Ok(Some(BorrowedComponentDeltaRecord {
            stable_key,
            replacement,
        }))
    }
...
impl<'a> Decoder<'a> {
    ...
    fn finish_ref(&self) -> Result<(), IndexError> {
...
mod tests {
    use super::*;
    use crate::v1::{CanonicalRecipeState, ProjectedDocumentState};
    ...
    #[test]
    fn replacement_keeps_exact_new_material_version() {
        let current = vec![state(19, b"stable", b"also stable")];
        let key = current[0].head.stable_key;
        let mut buffer = ProjectionMutationBuffer::new(16 * 1024).unwrap();
        buffer
            .apply_source_states([9; 32], "objects/a", 19, current)
            .unwrap();
        let sealed = buffer.seal().unwrap();
        let head = sealed
            .iter()
            .find(|s| s.component == ComponentIdentity::DocumentHead)
            .unwrap();
        ...
        .unwrap();
        assert_eq!(head.material_source_version, 19);
    ...
    #[test]
    fn repeated_complete_replacements_coalesce_bounded_components() {
        let mut buffer = ProjectionMutationBuffer::new(16 * 1024).unwrap();
        for version in 1..=10_001 {
            buffer
                .apply_state(&state(version, b"stable", b"also stable"))
                .unwrap();
        }
        let sealed = buffer.seal().unwrap();
        assert_eq!(sealed.len(), 4);
    ...
    #[test]
    fn later_replacement_overwrites_every_component() {
        let mut buffer = ProjectionMutationBuffer::new(16 * 1024).unwrap();
        buffer.apply_state(&state(2, b"stable", b"stable")).unwrap();
        buffer
            .apply_state(&state(3, b"changed", b"stable"))
            .unwrap();
        let sealed = buffer.seal().unwrap();
        assert_eq!(sealed.len(), 4);
        let field = sealed
            .iter()
            .find(|s| s.component == ComponentIdentity::Field(recipe(2)))
            .unwrap();
        assert_eq!(
            decode_component_delta_segment(&field.bytes)
                .unwrap()
                .records[0]
                .replacement
                .as_deref(),
    ...
    #[test]
    fn one_field_change_still_emits_complete_document() {
        let mut buffer = ProjectionMutationBuffer::new(16 * 1024).unwrap();
        buffer.apply_state(&state(2, b"new", b"stable")).unwrap();
        let sealed = buffer.seal().unwrap();
        assert_eq!(sealed.len(), 4);
        assert!(
            sealed
                .iter()
                .any(|s| s.component == ComponentIdentity::Field(recipe(3)))
        );
    }
    ...
    #[test]
    fn component_cursor_seeks_from_restart_table() {
        let records = (1..=160u8)
            .map(|value| {
                (
                    StableDocumentKey::from_bytes([value; 32]).unwrap(),
                    Some(vec![value]),
                )
            })
            .collect();
        let sealed = seal_component(ComponentIdentity::DocumentHead, records).unwrap();
        let mut cursor = ComponentDeltaCursor::new(&sealed.bytes).unwrap();
        for target in [1u8, 63, 64, 127, 128, 160] {
            let key = StableDocumentKey::from_bytes([target; 32]).unwrap();
            let record = cursor.seek_to(key).unwrap().unwrap();
            assert_eq!(record.stable_key, key);
            assert_eq!(record.replacement, Some([target].as_slice()));
        }
        assert!(
            cursor
                .seek_to(StableDocumentKey::from_bytes([255; 32]).unwrap())
                .unwrap()
                .is_none()
        );
    }
    ...
    #[test]
    fn deletion_without_predecessor_publishes_dead_head_and_locator_tombstone() {
        let mut buffer = ProjectionMutationBuffer::new(64 * 1024).unwrap();
        buffer
            .apply_source_states([9; 32], "objects/a", 2, Vec::new())
            .unwrap();
        let sealed = buffer.seal().unwrap();
        let locator = sealed
            .iter()
            .find(|s| s.component == ComponentIdentity::SourceRecords)
            .unwrap();
        assert!(
            decode_component_delta_segment(&locator.bytes)
                .unwrap()
                .records[0]
                .replacement
                .is_none()
    ...
    #[test]
    fn expanded_records_are_rejected_without_partial_publication() {
        let mut buffer = ProjectionMutationBuffer::new(64 * 1024).unwrap();
        let records = vec![
            state_record(2, 0, b"a", b"b"),
            state_record(2, 1, b"c", b"d"),
        ];
        assert!(
            buffer
                .apply_source_states([9; 32], "objects/a", 2, records)
                .is_err()
        );
        assert!(buffer.is_empty());
    }
    ...
    fn complete_source_admission_failure_rolls_back_every_component() {
        ...
        let mut buffer = ProjectionMutationBuffer::new(1024).unwrap();
        assert!(matches!(
            buffer.apply_source_states([9; 32], "objects/a", 2, current),
            Err(IndexError::ResourceLimit { .. })
        ));
        assert!(buffer.is_empty());
        assert_eq!(buffer.used_bytes(), 0);
