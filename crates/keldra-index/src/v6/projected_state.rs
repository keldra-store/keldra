    ...
    fn validate(&self) -> Result<(), IndexError> {
            ...
            || self.version == 0
        {
            return Err(IndexError::InvalidDefinition(
                "v6 object identity is invalid".into(),
            ));
        }
        Ok(())
    }
...
const QUERY_CACHE_IDENTITY_PREFIX: &str = "_keldra/index-cache/v6/";
...
fn decode_hex(value: u8) -> Result<u8, IndexError> {
    match value {
        ...
        _ => Err(IndexError::InvalidFormat(
            "format-v6 query-cache identity hex",
        )),
    }
}
...
impl StableDocumentKey {
    ...
    /// Query admission resolves the stable key through the pinned format-v6
    ...
    pub fn from_query_cache_identity(identity: &ObjectIdentity) -> Result<Self, IndexError> {
        let encoded = identity
            ...
            .ok_or(IndexError::InvalidFormat(
                "format-v6 query-cache identity prefix",
            ))?;
        if identity.version == 0 || encoded.len() != 64 {
            return Err(IndexError::InvalidFormat(
                "format-v6 query-cache identity shape",
            ));
        }
