...
pub(crate) fn public_scope_from_api(
    ...
    if storage_tenant.is_system()
        || realm.is_system()
        || realm.as_str() == keldra_authz::PERSONALDB_REALM_ID
    {
...
mod tests {
    ...
    fn public_scope_uses_trusted_tenant_and_internal_scope_keeps_system_support() {
        ...
        let personaldb = api::AuthzScope {
            storage_tenant: "acme".into(),
            realm: keldra_authz::PERSONALDB_REALM_ID.into(),
        };
        assert_eq!(
        ...
        assert_eq!(
            public_scope_from_api(Some(system.clone()), keldra_store::SYSTEM_STORAGE_TENANT_ID)
                .unwrap_err()
                .code(),
            Code::PermissionDenied
        );
        ...
        assert_eq!(
            public_scope_from_api(
                ...
                keldra_store::SYSTEM_STORAGE_TENANT_ID,
            )
            .unwrap_err()
            .code(),
            Code::PermissionDenied
