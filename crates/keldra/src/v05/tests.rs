...
fn content_type_is_bounded_by_utf8_bytes_before_a_put_token_can_be_issued() {
    ...
    let bulk_rejected = batch_operation(
        BulkOperation {
            operation: Some(keldra_api::v1::bulk_operation::Operation::PutIfVersion(
...
fn all_six_bulk_operations_are_explicit_and_zero_byte_puts_are_valid() {
    ...
    let operations = [
        keldra_api::v1::bulk_operation::Operation::Put(put()),
        keldra_api::v1::bulk_operation::Operation::PutIfAbsent(put()),
        keldra_api::v1::bulk_operation::Operation::PutIfVersion(BulkPutIfVersionRequest {
            address: address("object"),
            command_id: "command".into(),
            expected_version: 7,
            ..Default::default()
        }),
        keldra_api::v1::bulk_operation::Operation::PutImmutable(put()),
        keldra_api::v1::bulk_operation::Operation::Delete(ApiDeleteRequest {
            address: address("object"),
            command_id: "command".into(),
            ..Default::default()
        }),
        keldra_api::v1::bulk_operation::Operation::DeleteIfVersion(DeleteIfVersionRequest {
            address: address("object"),
            command_id: "command".into(),
            expected_version: 9,
            ..Default::default()
...
fn bulk_limit_accounts_for_every_encoded_operation_byte_without_cloning_payloads() {
    let operations = vec![
        BulkOperation {
            operation: Some(keldra_api::v1::bulk_operation::Operation::Put(
                BulkPutRequest {
                    address: address("one"),
                    bytes: vec![1, 2, 3],
        ...
        BulkOperation {
            operation: Some(keldra_api::v1::bulk_operation::Operation::Delete(
                ApiDeleteRequest {
                    address: address("two"),
...
fn bulk_metrics_partition_success_failure_and_replay_outcomes() {
    let response = Ok(Response::new(BulkWriteResponse {
        outcomes: vec![
            BulkOutcome {
                index: 0,
                outcome: Some(keldra_api::v1::bulk_outcome::Outcome::Receipt(
