...
pub(super) async fn put_end(
    ...
    if result.is_ok() && surface == UploadSurface::Public {
        tracing::debug!(
            monotonic_counter.keldra_object_publications_total = 1_u64,
            monotonic_counter.keldra_object_publication_payload_bytes_total = payload_bytes,
...
fn record_upload_phase<T>(
    ...
    tracing::debug!(
        monotonic_counter.keldra_object_upload_phase_requests_total = 1_u64,
        histogram.keldra_object_upload_phase_duration_seconds = started.elapsed().as_secs_f64(),
        phase = phase.as_str(),
    ...
    if phase == UploadPhase::Put {
        tracing::debug!(
            monotonic_counter.keldra_object_upload_received_bytes_total = payload_bytes,
            surface = surface.as_str(),
            result = outcome,
            grpc_status_code = status_code,
