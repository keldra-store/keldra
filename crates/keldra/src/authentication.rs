...
use keldra_consensus::JwtSigningKeyFingerprint;
use keldra_store::{CredentialSecretEnvelope, StorageTenantId};
...
const ACCESS_TOKEN_AUDIENCE: &str = "keldra-access";
...
const PUT_TOKEN_AUDIENCE: &str = "keldra-put";
const PUT_TOKEN_PURPOSE: &str = "put";
const JWT_SIGNING_KEY_FINGERPRINT_CONTEXT: &str = "keldra.auth/jwt-signing-key/v1";
const CREDENTIAL_ENVELOPE_KEY_CONTEXT: &str = "keldra.auth/s3-credential-envelope/aes256gcm/v1";
const PERSONALDB_SIGNING_MASTER_CONTEXT: &str = "keldra.personaldb/signing-master/v1";
...
impl Caller {
    ...
    pub(crate) fn authenticated_app_id(&self) -> Result<&str, AuthenticationError> {
        match &self.subject.id {
            keldra_authz::ObjectId::Opaque(app_id)
...
impl PersonalDbSigningPurpose {
    const fn context(self) -> &'static str {
        match self {
            Self::GroupControl => "keldra.personaldb/group-control/ed25519/v1",
            Self::ProjectionBuilder => "keldra.personaldb/projection-builder/ed25519/v1",
            Self::Snapshot => "keldra.personaldb/snapshot/ed25519/v1",
            Self::Witness => "keldra.personaldb/witness/ed25519/v1",
        }
    }
}
    ...
    pub(crate) fn mint_plugin_token(
        ...
        if bucket.is_empty()
            ...
            || path_prefix.split('/').any(|segment| {
                segment.is_empty() || segment == "." || segment == ".." || segment == "_keldra"
            })
        {
            return Err(AuthenticationError::InvalidIdentity(
    ...
    pub fn mint_put_token(
        ...
        let claims = PutTokenWireClaims {
            sub: match &caller.subject.id {
                keldra_authz::ObjectId::Opaque(app_id) => app_id.clone(),
                keldra_authz::ObjectId::ExactPath(_) => {
                    return Err(AuthenticationError::InvalidIdentity(
...
mod tests {
    ...
    use keldra_store::{PlacementLogId, SourceId, WatchScope};
    #[cfg(unix)]
    ...
    fn token_fields_cannot_encode_an_invalid_tenant_or_subject() {
        ...
        assert!(
            manager
                .mint(tenant("acme"), keldra_authz::PUBLIC_SUBJECT_ID)
                .is_err()
        );
        assert!(
            manager
                .mint(tenant("acme"), keldra_authz::ANONYMOUS_SUBJECT_ID)
                .is_err()
        );
    }
    ...
    fn signing_key_fingerprint_is_domain_separated_and_stable() {
        ...
        assert_eq!(
            ...
            JwtSigningKeyFingerprint(blake3::derive_key(
                "keldra.auth/jwt-signing-key/v1",
                TEST_SIGNING_SECRET,
            ))
        );
