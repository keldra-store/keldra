        ...
        loop {
            ...
            let (status, changes, returned_next) = tokio::task::spawn_blocking(move || {
                ...
                if status.source_id != expected_source {
                    return Err(keldra_store::WatchError::ResumeExpired);
                }
                let floor_next = status.retention_floor.checked_add(1).ok_or_else(|| {
                    keldra_store::WatchError::Storage("watch retention floor cannot advance".into())
                })?;
                let settled_next = status.settled_through.checked_add(1).ok_or_else(|| {
                    keldra_store::WatchError::Storage(
                        "watch settled boundary cannot advance".into(),
                    )
                })?;
                ...
                if let Some(oversize) = page.oversize {
                    return Err(keldra_store::WatchError::Storage(format!(
                ...
                let returned_next = page.through_offset.checked_add(1).ok_or_else(|| {
                    keldra_store::WatchError::Storage("watch cursor cannot advance".into())
                })?;
            ...
            .map_err(|error| match error {
                keldra_store::WatchError::ResumeExpired => WatchSourceError::ResumeExpired,
...
fn routed_watch_error(error: RoutedJournalError) -> keldra_store::WatchError {
    match error {
        RoutedJournalError::CursorExpired { .. }
        | RoutedJournalError::SourceEpochMismatch
        | RoutedJournalError::SourceNodeMismatch => keldra_store::WatchError::ResumeExpired,
        other => keldra_store::WatchError::Storage(other.to_string()),
    }
}
