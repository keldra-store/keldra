...
impl DistributedControlPlane {
    ...
    async fn read_and_verify_credential(
        ...
        secret: &str,
    ) -> Result<keldra_store::ApplicationCredential, Status> {
        let value = self
            .read_record(&LogicalRecordId::Credential {
                client_id: client_id.to_owned(),
            })
...
fn authz_status(error: keldra_store::AuthzStoreError) -> Status {
    Status::unavailable(error.to_string())
}

fn authz_evaluation_status(error: keldra_authz::AuthorizationError) -> Status {
    Status::internal(error.to_string())
}

fn credential_status(error: keldra_store::CredentialRepositoryError) -> Status {
    use keldra_store::CredentialRepositoryError::*;
    match error {
