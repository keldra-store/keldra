...
use keldra_consensus::{DecisionRaft, MembershipTransitionKind, NodeState};
use tonic::Status;
use tonic::body::Body;
...
mod tests {
    ...
    fn current_mutating_surfaces_are_classified() {
        assert!(is_public_mutation("/keldra.v1.ObjectService/PutEnd"));
        assert!(is_public_mutation("/keldra.v1.ObjectService/Delete"));
        assert!(is_public_mutation(
            "/keldra.v1.ObjectService/DeleteIfVersion"
        ));
        assert!(is_public_mutation("/keldra.v1.ObjectService/DeleteVersion"));
        assert!(is_public_mutation("/keldra.v1.ObjectService/BulkWrite"));
        assert!(is_public_mutation("/keldra.v1.AuthzService/MutateTuples"));
        assert!(is_public_mutation("/keldra.v1.IndexService/RebuildIndex"));
        assert!(is_public_mutation(
            "/keldra.v1.AdministrationService/RecoverApplicationCredential"
        ));
        assert!(!is_public_mutation("/keldra.v1.ObjectService/GetObject"));
        assert!(is_cluster_peer_mutation(
            "/keldra.cluster_peer.v1.ClusterPeer/RoutePutEnd"
        ));
        assert!(is_cluster_peer_mutation(
            "/keldra.cluster_peer.v1.ClusterPeer/RouteInternalBulkWrite"
        ));
        assert!(is_cluster_peer_mutation(
            "/keldra.cluster_peer.v1.ClusterPeer/RouteDeleteVersion"
        ));
        assert!(is_cluster_peer_mutation(
            "/keldra.cluster_peer.v1.ClusterPeer/RouteProvisionTenant"
        ));
        assert!(is_cluster_peer_mutation(
            "/keldra.cluster_peer.v1.ClusterPeer/RouteAdminRecoverCredential"
        ));
        assert!(!is_cluster_peer_mutation(
            "/keldra.cluster_peer.v1.ClusterPeer/ReadLogicalRecord"
        ));
    }
}
