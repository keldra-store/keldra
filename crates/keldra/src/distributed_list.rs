...
use keldra_authz::{ObjectRef, RealmId};
...
impl OriginalBearer {
    ...
    /// own; every source still evaluates Zanzibar for `app:_keldra/anonymous`.
    pub(crate) fn anonymous() -> Self {
        Self(Arc::from(keldra_authz::ANONYMOUS_SUBJECT_ID))
    }
    ...
    pub(crate) fn is_anonymous(&self) -> bool {
        self.0.as_ref() == keldra_authz::ANONYMOUS_SUBJECT_ID
    }
}
...
mod tests {
    ...
    use keldra_authz::{ObjectRef, Tuple};
    use keldra_consensus::{ClusterId, PeerAddress};
    use keldra_store::{
    ...
    fn startup_definition_discovery_is_bucket_scoped_and_excludes_general_heads() {
        ...
        assert!(!path_is_allowed_for_query(
            &query,
            "_keldra/accounting/12/definition.json"
        ));
    }
    ...
    async fn every_source_rechecks_jwt_current_grant_and_fence() {
        ...
        repository
            .mutate_tuples(TupleBatchRequest {
                scope: keldra_store::AuthzScope::system(),
