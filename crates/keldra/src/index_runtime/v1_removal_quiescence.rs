//! Pre-cutover proof that a departing source incarnation has a durable v1 tail.
...
pub(crate) struct V1RemovalQuiescenceProof {
    ...
    pub(crate) fn prove(
        ...
        if settled_next_offset == 0 {
            return Err(Status::failed_precondition(
                "v1 removal quiescence requires a settled source tail",
            ));
        }
        ...
        for entry in directory.entries.iter().filter(|entry| {
            ...
            let current = currents.get(&entry.partition).ok_or_else(|| {
                Status::failed_precondition(
                    "v1 removal quiescence lacks a required partition current",
                )
            })?;
            ...
            {
                return Err(Status::failed_precondition(
                    "v1 partition has not reached the settled removal cut",
                ));
            }
        ...
        if covered_partitions.is_empty() {
            return Err(Status::failed_precondition(
                "v1 removal quiescence found no directory partition for source",
            ));
        }
        Ok(Self {
            source,
...
fn index_status(error: keldra_index::IndexError) -> Status {
    Status::data_loss(format!("invalid v1 removal directory: {error}"))
}

#[cfg(test)]
mod tests {
    use super::*;
    use keldra_index::v1::{ProjectionPartitionDirectoryEntry, ProjectionPartitionLifecycle};
    ...
    fn caught_up_source_proves_its_exact_final_cut() {
        ...
        let proof =
            V1RemovalQuiescenceProof::prove(source(), 12, 9, &directory(), &currents).unwrap();
    ...
    fn lagging_partition_refuses_removal() {
        ...
        assert!(V1RemovalQuiescenceProof::prove(source(), 12, 9, &directory(), &currents).is_err());
    }
}
