//! Family directory and activation publication for the v1 physical catalog.
//!
...
use keldra_index::v1::{
    CatalogBaseline, CatalogOrdinalRange, ProjectionCatalogActivation,
    ProjectionFamilyPartitionDirectory, ProjectionPartitionDirectoryEntry,
    ProjectionPartitionIdentity, ProjectionPartitionLifecycle, QueryRecipeCatalogProof,
    RecipeIdentity, projection_catalog_routing_id,
};
use tonic::Status;
...
const LIFECYCLE_SAFETY_INTERVAL: Duration = Duration::from_secs(30);
const LIFECYCLE_RETRY_INTERVAL: Duration = Duration::from_secs(1);

pub(crate) struct V1CatalogLifecycleTask {
    task: tokio::task::JoinHandle<()>,
}

impl V1CatalogLifecycleTask {
    pub(crate) fn start(
        ...
        let task = tokio::spawn(async move {
            ...
            let mut interval = tokio::time::interval(LIFECYCLE_SAFETY_INTERVAL);
            ...
            loop {
                ...
                .await
                {
                    tracing::warn!(%error, "v1 projection catalog lifecycle will retry");
...
impl Drop for V1CatalogLifecycleTask {
    fn drop(&mut self) {
        self.task.abort();
    }
}
...
async fn reconcile_once(
    ...
    last_reconciled: &mut Option<(u64, IndexBarrier)>,
    forced: bool,
) -> Result<(), Status> {
    ...
    let input = (catalog_snapshot.generation, live_barrier.clone());
    if !forced && last_reconciled.as_ref() == Some(&input) {
        return Ok(());
    }
    let definition_catalog_hash = catalog_snapshot.identity;
    ...
    let live_generations = catalog_snapshot
        .recipes
        .iter()
        .map(|recipe| (recipe.family.family_id, recipe.physical_generation))
        .collect::<std::collections::BTreeSet<_>>();
    baselines
        .lock()
        .map_err(|_| Status::internal("v1 baseline catalog lock poisoned"))?
        .retain(|identity, _| live_generations.contains(identity));
    ...
    let (directory, directory_version) = if let Some((version, changed)) = directory_version {
        if changed {
            ...
            super::v1_telemetry::V1PipelineTelemetry::add(
                &super::v1_telemetry::global().catalog_directory_publications,
                1,
            );
...
async fn pinned_baseline(
    ...
    if let Some(existing) = baselines
        .lock()
        .map_err(|_| Status::internal("v1 catalog baseline registry is poisoned"))?
        .get(&key)
        .cloned()
    {
        return Ok(existing);
        ...
        .map(|(node, cursor)| {
            ...
            if fence != barrier.fence {
                return Err(Status::unavailable(
                    "v1 source producer assignment changed during captured barrier",
                ));
            }
...
fn extend_directory(
            ...
            match directory.entries[predecessor].lifecycle {
                ...
                ProjectionPartitionLifecycle::Retiring { .. } => {
                    return Err(Status::failed_precondition(
                        "v1 source incarnation has an unfinished different handoff",
                    ));
                }
            }
        } else {
    ...
    if changed {
        ...
        directory.revision = directory
            .revision
            .checked_add(1)
            .ok_or_else(|| Status::resource_exhausted("v1 family directory revision overflow"))?;
    }
...
async fn complete_covered_handoffs(
    ...
    loop {
        ...
        let successor_entry = directory
            ...
            .ok_or_else(|| {
                Status::data_loss("v1 retiring partition has no directory successor entry")
            })?;
        if successor_entry
...
async fn activate_if_complete(
        ...
        let source_baseline = baseline
            ...
            .ok_or_else(|| {
                Status::failed_precondition(
                    "v1 family directory partition is absent from the captured source barrier",
                )
            })?;
    ...
    let activation = ProjectionCatalogActivation {
        ...
        catalog_lineage: vec![recipe.physical_generation],
        baseline,
        required_partitions,
    ...
    projections
        ...
        .await?;
    super::v1_telemetry::V1PipelineTelemetry::add(
        &super::v1_telemetry::global().catalog_activations,
        1,
    );
    Ok(())
}
...
fn current_covers_cut(
    current: &keldra_index::v1::ProjectionCurrent,
...
fn activation_baseline(
    ...
    for entry in &directory.entries {
        let cursor = barrier
            ...
            .ok_or_else(|| {
                Status::failed_precondition(
                    "v1 family directory contains a partition outside the captured barrier",
                )
            })?;
    ...
    if baseline.is_empty() {
        return Err(Status::failed_precondition(
            "v1 family directory has no active partition baseline",
        ));
    }
    Ok(baseline)
}
...
fn index_status(error: keldra_index::IndexError) -> Status {
    Status::data_loss(format!(
        "invalid v1 projection catalog lifecycle evidence: {error}"
    ))
}
