//! Snapshot-bound initial backfill for one format-v1 source partition.
//!
...
pub(crate) struct V1BaselineSelected {
...
pub(crate) async fn open_partition_baseline(
        ...
        || snapshot.placement_fence().index != partition.placement_index
    {
        return Err(Status::unavailable(
            "v1 baseline snapshot belongs to another placement fence",
        ));
    }
    let checkpoint = snapshot
        ...
        .ok_or_else(|| Status::unavailable("v1 baseline source is not ACTIVE"))?;
    let captured_tail = checkpoint.captured_tail;
    let captured_next_offset = captured_tail
        .checked_add(1)
        .ok_or_else(|| Status::data_loss("v1 baseline source tail overflow"))?;
    Ok(V1PartitionBaseline {
        snapshot,
        recipe: recipe.clone(),
        source,
        captured_tail,
...
impl V1PartitionBaseline {
    ...
    pub(crate) async fn next_selected(
        ...
        if maximum_selected_bytes == 0 {
            return Err(Status::invalid_argument(
                "v1 baseline selected-object budget must be positive",
            ));
        }
        loop {
            if let Some(head) = self.pending.front().cloned() {
                ...
                self.next_baseline_offset = self
                    ...
                    .checked_add(1)
                    .ok_or_else(|| Status::data_loss("v1 baseline object count overflow"))?;
                if self.next_baseline_offset >= self.captured_next_offset {
                    return Err(Status::data_loss(
                        "v1 baseline contains more current source heads than journal positions",
                    ));
                }
                self.pending.pop_front();
                return Ok(Some(V1BaselineSelected {
                    baseline_offset,
    ...
    pub(crate) async fn next_selected_batch(
        ...
        maximum_selected_bytes: usize,
        max_items: usize,
    ) -> Result<Vec<V1BaselineSelected>, Status> {
        if max_items == 0 {
            return Err(Status::invalid_argument(
                "v1 baseline selected batch must allow at least one item",
            ));
        }
...
fn require_partition(
    ...
    {
        return Err(Status::data_loss(
            "v1 baseline recipe, source, and partition disagree",
        ));
    }
    Ok(())
}
...
fn baseline_object(
    ...
    let Some(stamp) = head.head.mutation_stamp else {
        ...
        // cannot be attributed to an exact v1 source partition.
        return Ok(None);
    };
    ...
    if stamp.source_journal_position > captured_tail {
        return Err(Status::data_loss(
            "v1 baseline head is newer than its captured source tail",
        ));
    }
    let blob = head
        .version
        .blob
        .ok_or_else(|| Status::data_loss("live v1 baseline source blob is absent"))?;
    Ok(Some((
        IndexBuildObject {
...
fn selected_resident_bytes(selected: &SelectedV1Source) -> Result<usize, Status> {
...
mod tests {
    ...
    #[test]
    fn unstamped_head_is_not_misattributed() {
        let expected = source(1);
        let mut unstamped = head("docs/a.json", expected, 1);
        unstamped.head.mutation_stamp = None;
        assert!(
            baseline_object(unstamped, expected, "docs/", Some("application/json"), 1,)
                .unwrap()
                .is_none()
        );
    }
