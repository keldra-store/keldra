...
impl keldra_index::IndexFileRead for ManifestArtifactFile {
    type Slice = IndexSlice;

    async fn read_at(&self, offset: u64, max_length: usize) -> Result<Self::Slice, IndexError> {
        keldra_index::IndexFileRead::read_at(&self.inner, offset, max_length).await
    }
}

impl keldra_index::v4::ArtifactDirectoryRead for ManifestArtifactDirectory {
