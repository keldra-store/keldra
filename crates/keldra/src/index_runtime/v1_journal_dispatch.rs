...
use super::v1_atomic_dispatch::{AtomicFinalizationDispatcher, FinalizedAtomicMutation};
...
/// segment boundary.
#[derive(Clone, Debug)]
pub(crate) struct V1FinalizedAtomicGroup {
...
#[derive(Clone, Debug)]
pub(crate) enum V1SourceDispatch {
    OrdinaryHead {
        source: SourceId,
        head: ObjectHeadChange,
    },
    FinalizedAtomic(V1FinalizedAtomicGroup),
}
...
pub(crate) struct V1OrderedSourceDispatcher {
...
impl V1OrderedSourceDispatcher {
    ...
    pub(crate) fn replace_assignment(
        ...
        if replacement < current {
            return Err(Status::failed_precondition(
                "v1 source assignment fence moved backward",
            ));
        }
        if replacement == current && assigned_sources != self.assigned_sources {
            return Err(Status::data_loss(
                "one placement fence assigned different v1 source incarnations",
            ));
        }
    ...
    pub(crate) fn observe(
        ...
        change: &LocalChange,
    ) -> Result<Vec<V1SourceDispatch>, Status> {
        ...
        for (source, mutations) in released {
            if self.is_assigned(source) {
                ...
                delivered.extend(groups.into_iter().map(|(cursor, mutations)| {
                    V1SourceDispatch::FinalizedAtomic(V1FinalizedAtomicGroup {
                        source,
                        cursor,
                        mutations,
                    })
    ...
    fn take_runnable_ordinary(&mut self, source: SourceId) -> Vec<V1SourceDispatch> {
        ...
        ready
            .into_values()
            .map(|head| V1SourceDispatch::OrdinaryHead { source, head })
            .collect()
    }
}
...
mod tests {
    ...
    use keldra_store::{
        AtomicBatchMutation, AtomicBatchPublished, ObjectHeadChangeKind, PlacementLogId,
    ...
    fn held(position: u64, cursor: u64) -> LocalChange {
    ...
    fn executor_event_releases_only_the_source_incarnation_assigned_here() {
        ...
        assert!(dispatcher.observe(remote, &held(5, 12)).unwrap().is_empty());
    ...
    fn only_non_program_heads_are_delivered_as_ordinary_source_work() {
        ...
        assert!(matches!(
            ordinary.as_slice(),
            [V1SourceDispatch::OrdinaryHead { head, .. }] if head.offset == 3
        ));
        assert!(dispatcher.observe(local, &held(4, 12)).unwrap().is_empty());
    ...
    fn ordinary_work_after_a_held_position_waits_for_the_contiguous_prefix() {
        ...
        assert!(matches!(
            delivered.as_slice(),
            [
                V1SourceDispatch::FinalizedAtomic(finalized),
                V1SourceDispatch::OrdinaryHead { head, .. }
    ...
    fn successor_can_receive_an_old_source_incarnation_after_handoff() {
        ...
        successor.observe(old_source, &held(4, 12)).unwrap();
