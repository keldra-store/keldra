    ...
    pub(crate) fn maximum_added_entries(&self, change: &LocalChange) -> Result<usize, Status> {
        match change {
            ...
            LocalChange::AtomicBatchPublished(batch) => batch
                ...
                .ok_or_else(|| Status::resource_exhausted("v1 atomic control backlog overflow")),
            _ => Ok(0),
        }
    }
...
mod tests {
    use keldra_store::{
        AtomicBatchPublished, ObjectHeadChange, ObjectHeadChangeKind, PreparedBundleHash, VersionId,
    };
    ...
    fn held(offset: u64, cursor: u64) -> LocalChange {
    ...
    fn remote_finalization_releases_each_source_only_as_one_complete_unit() {
        ...
        assert!(dispatcher.observe(a, &held(7, 41)).unwrap().is_empty());
        assert!(dispatcher.observe(b, &held(9, 41)).unwrap().is_empty());
    ...
    fn all_source_observation_drains_remote_source_owners() {
        ...
        let executor = source(3);
        let mut dispatcher = AtomicFinalizationDispatcher::default();
        dispatcher.observe(a, &held(7, 41)).unwrap();
        dispatcher.observe(b, &held(9, 41)).unwrap();
    ...
    fn finalization_seen_before_a_source_path_waits_for_the_hold() {
        ...
        let ready = dispatcher.observe(a, &held(8, 10)).unwrap();
    ...
    fn later_finalization_cannot_cross_an_earlier_source_hold() {
        ...
        let executor = source(2);
        let mut dispatcher = AtomicFinalizationDispatcher::default();
        dispatcher.observe(a, &held(7, 10)).unwrap();
        dispatcher.observe(a, &held(9, 11)).unwrap();
        dispatcher
    ...
    fn later_observation_cannot_advance_the_common_root_cut() {
        ...
        // A v1 root may therefore advance only through the proven cursor 10.
