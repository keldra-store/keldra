...
use keldra_store::{DefinitionCheckpoint, DefinitionKind, PlacementLogId, SourceId, Store};
use tonic::Status;
...
fn validate_membership_assignment_baseline(
    ...
    consumer_kind: keldra_store::DefinitionConsumerKind,
    source_node_id: u16,
    fence: PlacementLogId,
) -> Result<(), Status> {
...
mod tests {
    ...
    fn assignment_checkpoint(source_node_id: u16, fence: PlacementLogId) -> DefinitionCheckpoint {
        DefinitionCheckpoint {
            consumer_kind: keldra_store::DefinitionConsumerKind::IndexAssignments,
            source_id: SourceId {
    ...
    fn only_the_current_reconciliation_fence_releases_normal_delivery() {
        ...
        let kind = keldra_store::DefinitionConsumerKind::IndexAssignments;
        assert!(
            validate_membership_assignment_baseline(
    ...
    fn another_source_or_consumer_cannot_release_normal_delivery() {
        let current = PlacementLogId { term: 2, index: 4 };
        let index = keldra_store::DefinitionConsumerKind::IndexAssignments;
        assert_eq!(
            validate_membership_assignment_baseline(
        ...
        assert_eq!(
            validate_membership_assignment_baseline(
                Some(DefinitionCheckpoint {
                    consumer_kind: keldra_store::DefinitionConsumerKind::AccountingAssignments,
