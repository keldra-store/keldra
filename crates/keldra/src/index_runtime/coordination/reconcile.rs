...
fn queue_locator_assignments(
    kind: DefinitionKind,
    locator: &keldra_store::DefinitionLocator,
...
mod tests {
    ...
    use keldra_store::{DefinitionLocator, StoreOptions, VersionId};

    #[tokio::test]
    async fn membership_change_repairs_an_unassigned_locator_on_the_new_top_three() {
        let previous_fence = keldra_store::PlacementLogId { term: 2, index: 8 };
        let current_fence = keldra_store::PlacementLogId { term: 2, index: 9 };
    ...
    fn membership_reconciliation_rehydrates_deleted_definition_cleanup_on_current_owners() {
        let fence = keldra_store::PlacementLogId { term: 3, index: 9 };
        let locator = DefinitionLocator {
            kind: DefinitionKind::Index,
            tenant_id: 11,
            bucket_id: 12,
...
fn queue_cursorless_transition(
    transition: &keldra_store::DefinitionTransition,
    placement: &ClusterPlacement,
    by_destination: &mut BTreeMap<NodeId, Vec<DefinitionAssignmentMutation>>,
) -> Result<(), Status> {
    let owners = assignment_placement(
...
mod destination_barrier_tests {
    use keldra_store::{DefinitionConsumerKind, SourceId};

    use super::*;

    fn checkpoint(fence: keldra_store::PlacementLogId, next_offset: u64) -> DefinitionCheckpoint {
        DefinitionCheckpoint {
    ...
    fn an_old_fence_offset_never_skips_current_fence_baseline_installation() {
        let current = keldra_store::PlacementLogId { term: 3, index: 9 };
