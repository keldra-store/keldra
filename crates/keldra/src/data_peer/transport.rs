...
impl DataPeerTransport {
    ...
    pub(crate) async fn apply_object_mutation(
        ...
        Ok(ReplicaObjectMutationApplied {
            version: keldra_store::VersionId(response.version),
    ...
    pub(crate) async fn apply_object_mutation_batch(
        ...
        response
            ...
            .map(|outcome| {
                ...
                Ok(ReplicaObjectMutationApplied {
                    version: keldra_store::VersionId(outcome.version),
    ...
    ) -> Result<keldra_store::ReplicaRetainedVersionDeleteApplied, Status> {
        let response = self
            .client(target, address)?
            ...
            .into_inner();
        require_response_schema(response.schema_version)?;
        Ok(keldra_store::ReplicaRetainedVersionDeleteApplied {
    ...
    pub(crate) async fn put_small_content(
        ...
        if bytes.len() > keldra_store::SMALL_BLOB_MAX_BYTES
