...
pub(crate) async fn start(
    ...
    let (derived_progress, derived_retention) = DerivedConsumerRuntimeTask::start(
        keldra_store::DerivedConsumerKind::Accounting,
        local_node,
        decisions.clone(),
        store.clone(),
        event_journal.clone(),
...
#[cfg(test)]
mod tests {
    use keldra_store::{DefinitionAssignment, PlacementLogId, VersionId};
    ...
    async fn definitive_failed_revalidation_removes_stale_accounting_assignment() {
        ...
        let assignment = DefinitionAssignment {
            ...
            bucket_id: 3,
            definition_id: 4,
            definition_path: "_keldra/accounting/definitions/4".into(),
