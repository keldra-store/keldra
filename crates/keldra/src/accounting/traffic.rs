    ...
    fn emit_drop_state(&self, batches: u64, bytes: u64, reason: Option<&'static str>) {
        ...
        match reason {
            Some(reason) => tracing::warn!(
                ...
                dropped_bytes_total,
                monotonic_counter.keldra_accounting_traffic_dropped_batches_total = batches,
                monotonic_counter.keldra_accounting_traffic_dropped_bytes_total = bytes,
                reason,
                "keldra_accounting_traffic_drop_state"
            ),
            None => tracing::info!(
                node_id = self.source.node_id,
                dropped_batches_total,
...
fn emit_pending(queue: &TrafficQueue) {
    ...
    tracing::debug!(
        gauge.keldra_accounting_traffic_pending_batches =
            queue.ready.len().saturating_add(queue.open.len()) as u64,
        gauge.keldra_accounting_traffic_pending_bytes = queue.total_bytes,
        gauge.keldra_accounting_traffic_oldest_pending_millis = oldest.as_millis() as u64,
