...
impl StoredAccountingRollup {
    ...
    pub(crate) fn barrier(&self) -> Result<IndexBarrier, Status> {
        ...
        for source in &self.sources {
            let node = keldra_consensus::NodeId(source.node_id);
...
pub(crate) fn derive_accounting_id(tenant_id: u64, bucket_id: u64, prefix: &str) -> u64 {
    let mut hasher = blake3::Hasher::new_derive_key("keldra.accounting/definition-id/v1");
...
pub(crate) fn validate_prefix(prefix: &str) -> Result<(), Status> {
    ...
    if prefix.split('/').any(|segment| segment == "_keldra") {
        return Err(Status::invalid_argument(
            "accounting path prefix cannot select a reserved _keldra namespace",
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn prefix_matching_is_segment_aware_and_excludes_self_accounting() {
        ...
        assert!(!includes_path("", "_keldra/accounting/7/current"));
        assert!(includes_path("", "_keldra/indices/7/current"));
    }
    ...
    fn only_exact_reserved_shapes_are_artifacts() {
        assert!(is_artifact_path("_keldra/accounting/7/current", 7));
        assert!(is_artifact_path("_keldra/accounting/definitions/7", 7));
        assert!(is_artifact_path("_keldra/accounting/7/sources/2", 7));
        assert!(!is_artifact_path("_keldra/accounting/7/sources/02", 7));
        assert!(!is_artifact_path("_keldra/accounting/8/current", 7));
    }
