...
const METRIC_ATTRIBUTE_KEYS: &[&str] = &[
    ...
    "builder.phase",
    "recovery.action",
    "compaction.input_tier",
    "compaction.output_tier",
...
impl Observability {
    ...
    pub fn init(config: ObservabilityConfig) -> Result<Self> {
        ...
        let tracer = providers.tracer_provider.tracer("keldra-server");
        tracing_subscriber::registry()
...
fn otlp_trace_enabled(metadata: &tracing::Metadata<'_>) -> bool {
    ...
    metadata.target() != "keldra::index_runtime::cpu" || *metadata.level() != tracing::Level::DEBUG
}
    ...
    fn metric_attributes_are_bounded_and_compaction_active_cancels() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info!(
                ...
                counter.keldra_metric_attribute_filter_test = 1_i64,
                "a human-readable message must not become a metric attribute"
            );
            tracing::info!(
                index.kind = "typed_json",
                counter.keldra_index_query_waiting_filter_test = 1_i64
            );
            tracing::info!(
                index.kind = "typed_json",
                counter.keldra_index_query_waiting_filter_test = -1_i64
            ...
            tracing::info!(
                ...
                counter.keldra_index_query_outcome_filter_test = 1_i64
            );
            tracing::info!(
                operation = "serving_fence_renewal",
                counter.keldra_control_plane_active_filter_test = 1_i64
            );
            tracing::info!(
                operation = "serving_fence_renewal",
                counter.keldra_control_plane_active_filter_test = -1_i64
        ...
        let filtered = recorded
            .iter()
            .rev()
            .find(|sum| sum.name == "keldra_metric_attribute_filter_test")
        ...
        let query_waiting = recorded
            .iter()
            .rev()
            .find(|sum| sum.name == "keldra_index_query_waiting_filter_test")
        ...
        let query_outcome = recorded
            .iter()
            .rev()
            .find(|sum| sum.name == "keldra_index_query_outcome_filter_test")
        ...
        let control_active = recorded
            .iter()
            .rev()
            .find(|sum| sum.name == "keldra_control_plane_active_filter_test")
