...
use keldra_api::v1::index_service_server::IndexService as IndexServiceRpc;
    ...
    async fn load_listed_definitions(
            ...
            page.definitions.into_iter().zip(outcomes).enumerate()
        {
            ...
            let object = match outcome {
                keldra_api::v1::batch_get_outcome::Outcome::Object(object) => object,
                keldra_api::v1::batch_get_outcome::Outcome::Failure(failure)
...
impl IndexServiceRpc for IndexServiceImpl {
    ...
    async fn query_index(
        ...
        tracing::info!(
            ...
            histogram.keldra_index_query_request_duration_seconds = started.elapsed().as_secs_f64(),
...
mod tests {
    ...
    fn anonymous_query_requires_and_binds_an_explicit_tenant() {
        ...
        assert_eq!(
            ...
            &keldra_authz::ObjectRef::anonymous()
        );
        assert_eq!(context.routed_bearer(), keldra_authz::ANONYMOUS_SUBJECT_ID);
        assert_eq!(
