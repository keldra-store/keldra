...
#[path = "v1_index_resource_qualification/data.rs"]
mod data;
#[path = "v1_index_resource_qualification/incident.rs"]
mod incident;
#[path = "v1_index_resource_qualification/resource.rs"]
mod resource;
#[path = "v1_index_resource_qualification/singleton_probe.rs"]
mod singleton_probe;
...
async fn main() -> Result<()> {
    if let Some(path) = env::var_os("KELDRA_V1_RESOURCE_SINGLETON_PROBE_STATE_INPUT") {
        return singleton_probe::run(Path::new(&path)).await;
    }
    if let Some(path) = env::var_os("KELDRA_V1_RESOURCE_STATE_INPUT") {
...
fn qualification_index_request(bucket: &str) -> Result<CreateIndexRequest> {
    ...
    Ok(TypedJsonIndexBuilder::new(bucket, "records-by-field")
        ...
        .finish("v1-resource-create-index")?)
}

impl Config {
    fn from_env() -> Result<Self> {
        let records = number("KELDRA_V1_RESOURCE_RECORDS", DEFAULT_RECORDS)?;
        let mutation_count = number("KELDRA_V1_RESOURCE_MUTATIONS", DEFAULT_MUTATIONS)?;
        let batch_size = number("KELDRA_V1_RESOURCE_BATCH_SIZE", DEFAULT_BATCH_SIZE)?;
        let workers = number("KELDRA_V1_RESOURCE_WORKERS", DEFAULT_WORKERS)?;
        let verification_workers = number(
            "KELDRA_V1_RESOURCE_VERIFICATION_WORKERS",
        ...
        let endpoints = required("KELDRA_V1_RESOURCE_ENDPOINTS")?
            .split(',')
            .filter(|value| !value.is_empty())
            .map(str::to_owned)
            .collect::<Vec<_>>();
        ensure!(!endpoints.is_empty(), "at least one endpoint is required");
        let resource_pids = optional_list("KELDRA_V1_RESOURCE_PIDS")
            .into_iter()
            .map(|value| value.parse().context("invalid resource PID"))
            .collect::<Result<Vec<_>>>()?;
        let resource_containers = optional_list("KELDRA_V1_RESOURCE_CONTAINERS");
        let require_resource_targets = boolean("KELDRA_V1_REQUIRE_RESOURCE_TARGETS", true)?;
        let configured_kind_budget_bytes = optional_number("KELDRA_V1_KIND_BUDGET_BYTES")?;
        let configured_compaction_max_lanes =
            optional_number("KELDRA_V1_INDEX_COMPACTION_MAX_LANES")?;
        let configured_projection_max_lanes =
            optional_number("KELDRA_V1_INDEX_PROJECTION_MAX_LANES")?;
        let configured_rayon_workers = optional_number("KELDRA_V1_INDEX_RAYON_WORKERS")?;
        let max_anonymous_growth_bytes = optional_number("KELDRA_V1_MAX_ANONYMOUS_GROWTH_BYTES")?;
        let require_performance_targets = boolean("KELDRA_V1_REQUIRE_PERFORMANCE_TARGETS", false)?;
        let evidence = EvidenceConfig::from_env(endpoints.len())?;
        ensure!(
            configured_compaction_max_lanes.is_none_or(|lanes| lanes > 0),
            "KELDRA_V1_INDEX_COMPACTION_MAX_LANES must be non-zero when configured"
        );
        ensure!(
            configured_projection_max_lanes.is_none_or(|lanes| lanes > 0),
            "KELDRA_V1_INDEX_PROJECTION_MAX_LANES must be non-zero when configured"
        );
        if require_resource_targets {
                ...
                "resource qualification requires a non-zero KELDRA_V1_KIND_BUDGET_BYTES"
            );
            ensure!(
                configured_rayon_workers.is_some_and(|workers| workers > 0),
                "resource qualification requires a non-zero KELDRA_V1_INDEX_RAYON_WORKERS"
            );
            ensure!(
                ...
                "resource qualification requires KELDRA_V1_MAX_ANONYMOUS_GROWTH_BYTES"
            );
        }
        Ok(Self {
            endpoints,
            tenant: required("KELDRA_V1_RESOURCE_TENANT")?.into(),
            bucket: required("KELDRA_V1_RESOURCE_BUCKET")?.into(),
            client_id: required("KELDRA_V1_RESOURCE_CLIENT_ID")?,
            client_secret: required("KELDRA_V1_RESOURCE_CLIENT_SECRET")?,
            records,
            mutation_count,
            batch_size,
            workers,
            verification_workers,
            seed: number("KELDRA_V1_RESOURCE_SEED", DEFAULT_SEED)?,
            resource_pids,
            resource_containers,
            ...
            evidence,
            output: env::var_os("KELDRA_V1_RESOURCE_OUTPUT").map(PathBuf::from),
            state_output: env::var_os("KELDRA_V1_RESOURCE_STATE_OUTPUT").map(PathBuf::from),
        })
    }
}
...
impl EvidenceConfig {
    fn from_env(ingress_endpoint_count: usize) -> Result<Self> {
        let source_commit = required("KELDRA_V1_EVIDENCE_SOURCE_COMMIT")?;
        ensure!(
            source_commit.len() == 40 && source_commit.bytes().all(|byte| byte.is_ascii_hexdigit()),
            "KELDRA_V1_EVIDENCE_SOURCE_COMMIT must be a full Git commit ID"
        );
        let resolved_container_digest = required("KELDRA_V1_EVIDENCE_CONTAINER_DIGEST")?;
        let digest_hex = resolved_container_digest
            .strip_prefix("sha256:")
            .context("KELDRA_V1_EVIDENCE_CONTAINER_DIGEST must use sha256")?;
        ensure!(
            digest_hex.len() == 64 && digest_hex.bytes().all(|byte| byte.is_ascii_hexdigit()),
            "KELDRA_V1_EVIDENCE_CONTAINER_DIGEST must be a complete sha256 digest"
        );
        let native_architecture = required("KELDRA_V1_EVIDENCE_NATIVE_ARCHITECTURE")?;
        ensure!(
            !native_architecture.trim().is_empty(),
            "native architecture evidence must not be empty"
        );
        let container_platform = required("KELDRA_V1_EVIDENCE_CONTAINER_PLATFORM")?;
        ensure!(
            matches!(container_platform.as_str(), "linux/amd64" | "linux/arm64"),
            "container platform evidence must be linux/amd64 or linux/arm64"
        );
        let topology = required("KELDRA_V1_EVIDENCE_TOPOLOGY")?;
        let node_count = required_number("KELDRA_V1_EVIDENCE_NODE_COUNT")?;
        ensure!(
            matches!(
        ...
        let hardware_logical_cpus = required_number("KELDRA_V1_EVIDENCE_HARDWARE_LOGICAL_CPUS")?;
        let hardware_memory_bytes = required_number("KELDRA_V1_EVIDENCE_HARDWARE_MEMORY_BYTES")?;
        let qualification_filesystem_total_bytes =
            required_number("KELDRA_V1_EVIDENCE_FILESYSTEM_TOTAL_BYTES")?;
        let qualification_filesystem_available_bytes_at_start =
            required_number("KELDRA_V1_EVIDENCE_FILESYSTEM_AVAILABLE_BYTES")?;
        let index_disk_cache_bytes_per_node =
            required_number("KELDRA_V1_EVIDENCE_INDEX_DISK_CACHE_BYTES_PER_NODE")?;
        let index_memory_percent_per_node =
            required_number("KELDRA_V1_EVIDENCE_INDEX_MEMORY_PERCENT_PER_NODE")?;
        ensure!(
...
fn command_id(mode: MutationMode, record_id: u64) -> String {
    ...
    format!("v1-resource-{label}-{record_id}")
}
...
async fn verify_existing_state(path: &Path) -> Result<()> {
    ...
    let tenant = required("KELDRA_V1_RESOURCE_TENANT")?;
    let bucket = required("KELDRA_V1_RESOURCE_BUCKET")?;
    ensure!(
        tenant == state.tenant,
    ...
    let client_id = required("KELDRA_V1_RESOURCE_CLIENT_ID")?;
    let client_secret = required("KELDRA_V1_RESOURCE_CLIENT_SECRET")?;
    let verification_workers = number(
        "KELDRA_V1_RESOURCE_VERIFICATION_WORKERS",
        DEFAULT_VERIFICATION_WORKERS,
    )?;
