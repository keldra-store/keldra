...
case "${mode}" in
  ...
  mixed)
    mixed_workload=1
    definition_matrix="${KELDRA_V1_SCALE_DEFINITION_MATRIX:-1}"
    recipe_matrix="${KELDRA_V1_SCALE_RECIPE_MATRIX:-1}"
    worker_matrix="${KELDRA_V1_SCALE_WORKER_MATRIX:-4}"
    memory_per_worker_matrix="${KELDRA_V1_SCALE_MEMORY_PER_WORKER_MATRIX:-1073741824}"
    rate_ladder="${KELDRA_V1_SCALE_MIXED_TOTAL_OPERATIONS_PER_SECOND:?set KELDRA_V1_SCALE_MIXED_TOTAL_OPERATIONS_PER_SECOND}"
    object_size_matrix="${KELDRA_V1_SCALE_OBJECT_SIZE_MATRIX:-1024}"
    baseline_seconds="${KELDRA_V1_SCALE_BASELINE_SECONDS:-30}"
    mixed_phase_seconds="${KELDRA_V1_SCALE_MIXED_PHASE_SECONDS:-7200}"
    concurrent_seconds=$((mixed_phase_seconds * 2))
    post_seconds="${KELDRA_V1_SCALE_POST_SECONDS:-30}"
    ;;
  *) echo "KELDRA_V1_SCALE_MODE must be smoke, sustained, or mixed" >&2; exit 2 ;;
esac
...
if ((mixed_workload == 1)); then
  [[ "${rate_ladder}" =~ ^[1-9][0-9]*0$ ]] || {
    echo "mixed total operation rate must be one positive multiple of 10" >&2
    exit 2
  }
fi
...
positive_integer "${mutable_records}" && ((mutable_records <= 1000000)) || {
...
block_device_sampler_pid=""
{
  ...
  echo "mode=${mode}"
  echo "mixed_workload=${mixed_workload}"
  if ((mixed_workload == 1)); then
    echo "mixed_phase_seconds=${mixed_phase_seconds}"
    echo "mixed_ratio_sequence=70-write-30-read,30-write-70-read"
  fi
...
stop_block_device_sampler() {
  local status=0
  [[ -n "${block_device_sampler_pid}" ]] || return 0
  : >"${block_device_sampler_stop}"
  wait "${block_device_sampler_pid}" || status=$?
  block_device_sampler_pid=""
...
cleanup() {
  ...
  trap - EXIT INT TERM
  stop_server
  stop_block_device_sampler || status=1
...
summarize_cell() {
  ...
  window_start_ms="$(jq -er '(.mutations // .partial_evidence.mutations).measurement_window_started_unix_milliseconds' "${report}")"
  window_end_ms="$(jq -er '(.mutations // .partial_evidence.mutations).measurement_window_ended_unix_milliseconds' "${report}")"
  ...
  jq -cn --arg cell "${cell}" --arg report_path "${report}" --arg progress_path "${progress}" --arg telemetry_path "${telemetry_samples}" --arg block_device_samples "${block_device_samples}" \
    --argjson definitions "${definitions}" --argjson recipes "${recipes}" --argjson workers "${workers}" \
    --argjson memory_per_worker "${memory_per_worker}" --argjson query_memory_bytes "${query_memory_bytes}" --argjson mutation_workers "${mutation_workers}" --argjson mutable_records "${mutable_records}" --argjson object_bytes "${object_bytes}" \
    ...
    for workers in "${worker_values[@]}"; do
      for memory_per_worker in "${memory_values[@]}"; do
        ...
        for object_bytes in "${object_sizes[@]}"; do
          ...
          cell="d${definitions}-p${recipes}-w${workers}-m${memory_per_worker}-b${object_bytes}-n${mutable_records}-r${target_data_rate//./_}-cw${mutation_workers}"
          ...
          if ((mixed_workload == 1)); then
            driver_env+=(
              "KELDRA_INDEX_CONTENTION_MIXED_TOTAL_OPERATIONS_PER_SECOND=${target_data_rate}"
              "KELDRA_INDEX_CONTENTION_MIXED_PHASE_SECONDS=${mixed_phase_seconds}"
            )
          else
            driver_env+=(
