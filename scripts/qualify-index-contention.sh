...
visibility_query_rate="${KELDRA_INDEX_CONTENTION_VISIBILITY_QUERY_RATE:-20}"
...
for timeout_value in "${request_timeout_ms}" "${drain_timeout_seconds}" \
  ...
  "${visibility_query_rate}" "${visibility_sample_every_batches}" "${mutation_workers}" \
...
jq -n \
  ...
  --argjson visibility_query_rate "${visibility_query_rate}" \
...
run_qualification_driver() {
...

  {
    ...
    printf 'export KELDRA_INDEX_CONTENTION_VISIBILITY_QUERY_RATE=%q\n' "${visibility_query_rate}"
  ...
  for definitions in "${definition_matrix[@]}"; do
    jq -cn \
      ...
      {index_definition_count:$definition_count,
       outcomes:{baseline:{result:$before[0].result,responsiveness:$before[0].responsiveness,mutations:{fully_successful_batches:$before[0].mutations.fully_successful_batches,structurally_valid_batches_with_operation_failures:$before[0].mutations.structurally_valid_batches_with_operation_failures,indeterminate_batches:$before[0].mutations.indeterminate_batches,undispatched_at_measurement_deadline_batches:$before[0].mutations.undispatched_at_measurement_deadline_batches,undispatched_at_measurement_deadline_data_operations:$before[0].mutations.undispatched_at_measurement_deadline_data_operations,successful_data_operations:$before[0].mutations.successful_data_operations,failed_data_operations:$before[0].mutations.failed_data_operations,indeterminate_data_operations:$before[0].mutations.indeterminate_data_operations,successful_probe_operations:$before[0].mutations.successful_probe_operations,failed_probe_operations:$before[0].mutations.failed_probe_operations,indeterminate_probe_operations:$before[0].mutations.indeterminate_probe_operations},concurrent:{scheduled:$before[0].concurrent.scheduled_queries,successful:$before[0].concurrent.successful_queries,scheduler_deadline_misses:$before[0].concurrent.scheduler_deadline_misses,client_concurrency_rejections:$before[0].concurrent.client_concurrency_rejections,request_errors:$before[0].concurrent.request_errors,timeouts:$before[0].concurrent.timeouts}},candidate:{result:$after[0].result,responsiveness:$after[0].responsiveness,mutations:{fully_successful_batches:$after[0].mutations.fully_successful_batches,structurally_valid_batches_with_operation_failures:$after[0].mutations.structurally_valid_batches_with_operation_failures,indeterminate_batches:$after[0].mutations.indeterminate_batches,undispatched_at_measurement_deadline_batches:$after[0].mutations.undispatched_at_measurement_deadline_batches,undispatched_at_measurement_deadline_data_operations:$after[0].mutations.undispatched_at_measurement_deadline_data_operations,successful_data_operations:$after[0].mutations.successful_data_operations,failed_data_operations:$after[0].mutations.failed_data_operations,indeterminate_data_operations:$after[0].mutations.indeterminate_data_operations,successful_probe_operations:$after[0].mutations.successful_probe_operations,failed_probe_operations:$after[0].mutations.failed_probe_operations,indeterminate_probe_operations:$after[0].mutations.indeterminate_probe_operations},concurrent:{scheduled:$after[0].concurrent.scheduled_queries,successful:$after[0].concurrent.successful_queries,scheduler_deadline_misses:$after[0].concurrent.scheduler_deadline_misses,client_concurrency_rejections:$after[0].concurrent.client_concurrency_rejections,request_errors:$after[0].concurrent.request_errors,timeouts:$after[0].concurrent.timeouts}}},
