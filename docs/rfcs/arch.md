...
Status: current high-level architecture for Keldra 0.18
...
6. Correctness-critical metadata is committed synchronously. Rebuildable and
   ...
   it.
7. Keldra 0.18 is a clean persistence generation. It does not provide backward
   compatibility with earlier on-disk formats.
...
A fresh cluster starts with one Raft voter and records the current Keldra 0.18
...
Keldra 0.18 uses its current compact durable codecs and storage markers. Earlier
volumes are not opened or migrated in place. Operators must start 0.18 with new
...
- Keldra 0.18 requires fresh storage; there is no in-place compatibility path
