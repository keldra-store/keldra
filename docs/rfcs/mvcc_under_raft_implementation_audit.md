...
| Committed object metadata must carry a canonical shard manifest and support verified reconstruction | Partial | `PhysicalObjectShardManifest::from_ingest` binds placements, incarnation, failure domain, transfer identity and hashes; its reference is added to the transaction bundle. Range reconstruction fetches only shard transfers and Reed-Solomon reconstructs missing data. The combined E2E commits the manifest, removes a target, and reconstructs from surviving shards. A current defect remains: ingest records BLAKE3 shard hashes, while `read_range_chunks` compares fetched bytes with SHA-256 before reconstruction. |
