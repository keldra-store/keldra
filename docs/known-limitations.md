...
| Sparse ordered predicates | Physical-order execution stops as soon as it has an exact authorized page, but a sparse conjunction can still advance through more than one quarter of a small segment before finding that page. A 16,384-object qualification returned the exact limit-4 result after 5,320 cursor advances and 2.27 MiB of logical reads. Pagination, bounded memory, authorization, and result order remain exact. Adding a more selective indexed predicate or narrowing the index path reduces this work; stronger ordered/posting skipping is deferred to production-driven planner work. |
...
| Typed/Metadata conjunctions | Every predicate is enforced and each individual equality, `IN`, prefix, range, or `EXISTS` lookup uses bounded posting ranges. A conjunction estimates those ranges from routing metadata, chooses the smallest candidate driver, and checks the remaining predicates against its projected candidates instead of intersecting several compressed posting streams. Ordered queries seek through the first order field when every run contains one comparable scalar type; missing or mixed order values use the exact driver fallback and can read more candidates. Complete, consistently typed order fields and selective predicates provide the fastest path; range-local multi-stream intersection is deferred to a later index format. |
| Query placement | One weighted-HRW owner executes a query. Large indices may fetch ordinary pack objects, but there is no scatter/gather query engine. |
