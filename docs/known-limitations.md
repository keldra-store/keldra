...
| Typed JSON Boolean expression bounds | Public Typed JSON and Metadata Filter predicates support nested conjunction, disjunction, and negation. One expression is limited to 32 levels of depth and 256 total nodes; empty Boolean operators are rejected. Negation is set complement over indexed live documents rather than SQL three-valued logic. `IN` remains the compact bounded same-field disjunction. |
